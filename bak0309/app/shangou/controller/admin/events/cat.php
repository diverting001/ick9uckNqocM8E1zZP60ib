<?php
class shangou_ctl_admin_events_cat extends desktop_controller {
	...
	function index() {
		...
		if ($tree) {
			foreach ( $tree as $k => $v ) {
				$tree [$k] ['link'] = array (
						'cat_id' => array (
								'v' => $v ['cat_id'],
								't' => app::get ( 'shangou' )->_ ( '商品类别' ) . app::get ( 'shangou' )->_ ( '是' ) . $v ['cat_name'] 
						) 
				);
			}
		}
		...
		$this->page ( 'admin/events/category/map.html' );
	}
	...
	function _info($id = 0, $type = 'add') {
		...
		$this->display ( 'admin/events/category/info.html' );
	}
	function save() {
		$this->begin ( 'index.php?app=shangou&ctl=admin_events_cat&act=index' );
		if ($_POST ['p_order'] === '')
			$_POST ['p_order'] = 0;
		
		$objCat = &$this->app->model ( 'cat' );
		if ($objCat->save ( $_POST ['cat'] )) {
			$this->end ( true, app::get ( 'shangou' )->_ ( '保存成功' ) );
		} else {
			$this->end ( false, app::get ( 'shangou' )->_ ( '保存失败' ) );
		}
	}
	function toRemove($nCatId) {
		...
		if ($objCat->toRemove ( $nCatId, $msg )) {
			$this->end ( true, $cat_sdf ['cat_name'] . app::get ( 'shangou' )->_ ( '已删除' ) );
		}
		$this->end ( false, $msg );
	}
	...
	function update() {
		$this->begin ( 'index.php?app=shangou&ctl=admin_events_cat&act=index' );
		$o = $this->app->model ( 'cat' );
		if (empty($_POST ['p_order'])) {
			$this->end ( true, app::get ( 'shangou' )->_ ( '操作成功' ) );
		}
	...
	function get_subcat($cat_id) {
		...
		$this->display ( 'admin/events/category/cat_list.html' );
	}
}
