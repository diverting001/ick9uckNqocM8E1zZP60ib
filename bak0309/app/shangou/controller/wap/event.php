...
class shangou_ctl_wap_event extends wap_frontpage{
    ...
    public function  index($evt_id = null){
    	...
    	//判断是否设置了模版文件
    	if($event['wap_enevt_template']){
    		$this->set_tmpl_file($event['wap_enevt_template']);                 //添加模板
    	}
    	
    	$this->page('wap/detail/index.html');
    }
    ...
    * */
    private function screen($cat_id,$filter,$goodsList){
    	...
    	foreach ($goodsList as $g){
    		$value_ids = $g['products']['spec_desc']['spec_value_id'];
    		foreach ($value_ids as $k=>$v){
    			if(in_array($v,$allSpecV)){
    				continue;
    			}else{
    				$allSpecV[$k] = $v;
    			}
...
//     	foreach ($SpecList as $k1=>$v_sl){
//     		foreach ($v_sl['spec_value'] as $k2=>$v_sv){
//     			if(in_array($v_sv['spec_value'],$allSpecV)){
//     				continue;
//     			}else{
//     				//unset($SpecList[$k1]['spec_value'][$k2]);
//     			}
//     		}
//     	}
...
//     	}
    ...
    * */
    public function ajax_get_goods(){
    	...
    	if($goodsData){
    		...
    		$view = 'wap/detail/type/grid.html';
    		echo $this->fetch($view);
    	}else{
    		//后台站点设置搜索为空页面
    		echo $this->fetch('wap/detail/empty.html');
    	}
    }
    ...
    * */
	...
	function origin() {
		...
		$filter = array('leho_type'=>'true');
		$obj_events = app::get('shangou')->model ( 'events' );
		$obj_events_goods_item = app::get ( 'shangou' )->model ( 'events_goods_item' );
		$goods_mdl = &app::get ( 'b2c' )->model ( 'goods' );
		// 购活动基础信息
		$events = $obj_events->getList ("*", $filter );
		$goodslist = array();
		if(!empty($events)){
			$event = $events[0];
			$evt_id = $event['evt_id'];
			// 购活动商品信息
			$goods_array = $obj_events_goods_item->getList ( 'goods_id,d_order', array (
					'evt_id' => $evt_id 
			) );
			...
			$goodslist = $goods_mdl->getlist ( 'cat_id,goods_id,bn,name,price,mktprice,image_default_id,nostore_sell,spec_desc,type_id', array (
					'goods_id' => $goodids 
			), 0, 9999999, ' uptime desc ' );
			$goodslist = $this->get_product ( $goodids, $goodslist, $order );
		}
		$this->pagedata ['goods'] = $goodslist;
		$html = $this->fetch('wap/index/origin.html');
		echo $html;
	}
}
