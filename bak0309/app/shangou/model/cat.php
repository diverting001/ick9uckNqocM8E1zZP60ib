...
class shangou_mdl_cat extends dbeav_model{
    ...
    public function getTree(){
        return $this->db->select('SELECT o.cat_name AS text,o.cat_id AS id,o.parent_id AS pid,o.p_order,o.cat_path,
                    is_leaf,o.child_count FROM '.$this->table_name(1).' o   ORDER BY o.p_order,o.cat_id');
    }
    ...
    public function getCatParentById($id,$view='index'){
        if(!$id) return false;
            ...
            foreach($result as $cat_key=>$cat_value){
                $result[$cat_key]['link'] = app::get('site')->router()->gen_url(array('app'=>'shangou','ctl'=>'site_events','args'=>array($cat_value['cat_id'],$default_view) ));
            }
            return $result;
     }
    ...
    public function getMap($depth=-1,$cat_id=0){
        ...
        foreach($rows as $k=>$row){
            if($depth<0 || substr_count($row['cat_path'],',') < $depth){
                $cats[$row['cat_id']] = array('type'=>'gcat','parent_id'=>$row['parent_id'],'title'=>$row['cat_name'],'link'=>app::get('site')->router()->gen_url(array('app'=>'shangou', 'ctl'=>'site_events','act'=>'index','args'=>array($row['cat_id']) )));
            }
        }
    ...
    public function getPath($catId,$method=null){
        ...
        $row = $list_row[0];
        $ret = array(array('type'=>'goodsCat','title'=>$row['cat_name'],'link'=>app::get('site')->router()->gen_url(array('app'=>'shangou', 'ctl'=>'site_events','act'=>'index','args'=>array($cat_id['cat_id']) ))));
        if($row['cat_path'] != ',' && $row['cat_path']){
            ...
            foreach($rows as $row){
                array_unshift($ret,array('title'=>$row['cat_name'],'link'=>app::get('site')->router()->gen_url(array('app'=>'shangou', 'ctl'=>'site_events','act'=>'index','args'=>array($row['cat_id']) ))   ));
            }
        }
    ...
    function map($data,$sID=0,$preStr='',&$cat_cuttent,&$step){
        ...
        if($data){
            ...
            foreach($data as $i=>$value){
                ...
                $link=app::get('site')->router()->gen_url(array('app'=>'shangou','ctl'=>'site_events','args'=>array($id) ));
    ...
    function cat2json($return=false){
        ...
        base_kvstore::instance('shangou_events')->store('events_cat.data',$contents);
        app::get('shangou')->setConf('kvstore_events_cat_expires',time());
        if($return){
            return $contents;
        }else{
            return true;
    ...
    function get_cat_list($show_stable=false){
        if( base_kvstore::instance('shangou_events')->fetch('events_cat.data', $contents) !== false ){
            ...
            if($result){
                ...
                foreach($result as $k=>$val){
                    $result[$k]['url'] = app::get('site')->router()->gen_url(array('app'=>'shangou','ctl'=>'site_events','args'=>array($val['cat_id']) ));
                }
    ...
    function toRemove($catid,&$msg=''){
		...
		$aEvents = $obj_events->getList('evt_id',array('cat_id'=>intval($catid),'disabled'=>'false'));
        if(count($aEvents) > 0){
            $msg = '删除失败：本频道下面还有活动';
            return false;
        }
        ...
        $this->db->exec('DELETE FROM '.$this->table_name(1).' WHERE cat_id='.intval($catid));
        $this->db->exec('UPDATE '.$this->table_name(1).' SET child_count = child_count-1 WHERE cat_id='.intval($parent_id));
        $this->cat2json();
        return true;
    }

    function get_new_cat($limit){
        $cat_id = $this->db->select('SELECT cat_id FROM '.app::get('shangou')->model('events')->table_name(1).'  where cat_id <> \'0\' group by cat_id order by evt_id desc limit 0,'.$limit);
        if(is_array($cat_id)){
