...
class timedbuy_promotion_solution_timedbuy
{
    ...
    public function apply(&$object,$aConfig,&$cart_object = null) {
        if(is_null($cart_object)) { // 商品预过滤
            ...
            //添加库存限制
            if (!empty($aConfig['limit'])) {
            	$limit  = intval($aConfig['limit']) >=1 ? intval($aConfig['limit']):intval($aConfig['quantity']) ;
            	if ($object['obj_items']['products'][0]['store'] > $limit) {
            		$object['obj_items']['products'][0]['store'] =  $limit;
            		$object['store']['store'] = $object['obj_items']['products'][0]['store'];
            		$object['store']['real'] = $object['obj_items']['products'][0]['store'];
            	}
            }
            if(!empty($aConfig['min'])){
                $min = $aConfig['min']?$aConfig['min']:'';
                $object['store']['min'] = $min;
            }
