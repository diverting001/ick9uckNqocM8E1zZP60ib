...
class site_router implements base_interface_router
{
    ...
    public function gen_url($params = array())
    {
        ...
        if(!is_null($this->get_urlmap($params['app'].':'.$params['ctl']))){
            ...
            if(!isset($this->__gen_url_array[$gen_key])){
             ...
             return $this->filter_crlf($this->__gen_url_array[$gen_key]);
    ...
    public function default_gen_url($params=array())
    {
        ...
        if($params['act']=='index' && (count($params['args'])==0 || is_numeric($first_arg) || $first_arg=="'+pid+'")){
    ...
    public function http_status($code,$message='')
    {
        ...
        if($msg){
            $msg.='_Msg='.$message;
            kernel::single('site_controller')->splash('failed',null,$msg);
        }
        exit;
    }//End Function
    ...
    private function check_https() {
        ...
        /* } */
        $use_ssl_control = true;
        $server = kernel::request()->get_host();
        //$this->get_current_sitemap('use_ssl') === false
        if ($_SERVER['HTTP_X_FORWARDED_PORT'] == '8080' && kernel::single('site_route_sslwhitelist')->ChekSSl($this->get_query_info('query')) === true) {
            header("Location:https://" . $server . $_SERVER['REQUEST_URI']);
            exit();
        } else if ($_SERVER['HTTP_X_FORWARDED_PORT'] == '8081' && kernel::single('site_route_sslwhitelist')->ChekSSl($this->get_query_info('query')) === false) {
            //去掉https强转http逻辑
//            $compay_id = kernel::single("b2c_member_company")->get_cur_company();
//            $companyConf = kernel::single("b2c_global_scope")->getScopeByCompany('company', $compay_id, 'no_coercion_http');
//            if (isset($companyConf['key_value']) && $companyConf['key_value']) {
//                return;
//            } else {
        ...
        **/
        ...
        if(!$this->is_need_cache() || ($this->is_need_cache() && !cachemgr::get($page_key, $page, $router_cache_options['skipvary']))){
            ...
            if(WITHOUT_CACHE!==true){
                logger::debug('page cache miss:'.$this->_request->get_request_uri());
            }
            cachemgr::co_start();
            $this->default_dispatch();
            $page['html'] = join("\n", $this->_response->get_bodys());
            $page['date'] = date("Y-m-d H:i:s");
            $page['mysql_queries'] = base_db_connections::$mysql_query_executions;
			$page['kvstore_querys'] = base_kvstore::$__fetch_count;
        ...
        logger::debug('Mysql queries count:' . $page['mysql_queries']);
        logger::debug('Kvstore queries count:' . $page['kvstore_querys']);
    ...
    public function default_dispatch()
    {
        ...
        if(method_exists($controller, $action)){
            try{
                ...
                $this->http_status(405, $this->_request->get_ctl_name());   //405页面
            }
        }else{
