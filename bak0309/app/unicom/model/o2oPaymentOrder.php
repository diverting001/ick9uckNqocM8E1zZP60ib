 ...
 * Time: 7:52 PM
 */
class unicom_mdl_o2oPaymentOrder extends base_db_external_model
{
    // 数据库配置
    ...
    public function getOrderRaw($whereArr = array()){
        ...
        $sql = 'select * from unicom_o2o_order '.$where;
    ...
    public function add($data = array()){
        ...
        $values = "'".implode("','",$data)."'";
        $sql = 'insert into unicom_o2o_order ('.$fields.') '.'values('.$values.')';
    ...
    public function update($whereArr = array(),$data = array()){
        ...
        $sql = 'UPDATE `unicom_o2o_order` SET '.$set.$where;
        $result = $this->_db->exec($sql);
        if ($result['rs']) {
            return true;
        }
