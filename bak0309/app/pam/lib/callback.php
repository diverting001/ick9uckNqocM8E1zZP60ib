        ...
        if($params['module']){
            if(class_exists($params['module']) && ($passport_module = kernel::single($params['module']))){
                if($passport_module instanceof pam_interface_passport){
                    ...
                    if($module_uid){
                        $_SESSION['LOGIN'] = $_SESSION['LOGIN_TMP'];
                        unset($_SESSION['LOGIN_TMP']);
                    ...
                    if($module_uid && $_COOKIE['S']['SIGN']['AUTO'] > 0){
                        $minutes = 30*24*60;
                        //$minutes = 10*24*60;
                        kernel::single('base_session')->set_cookie_expires($minutes);
                        $_SESSION['AUTOLOGIN']['EXPIRES'] = time() + $minutes*60;
                        kernel::single('base_session')->set_sess_expires($minutes);

                    }elseif($params['expires']){
                        $minutes = ($params['expires'] - time()) / 60;
                        $_SESSION['AUTOLOGIN']['EXPIRES'] = $params['expires'];
                        $_SESSION['session_ttl'] = $params['expires'];
                        kernel::single('base_session')->set_cookie_expires(30*24*60);
                        kernel::single('base_session')->set_sess_expires($minutes);
                    }else{
                        $minutes = 10*24*60;
                        kernel::single('base_session')->set_cookie_expires($minutes);
                        kernel::single('base_session')->set_sess_expires(120);
                    }
