...
class pam_account{
	...
	*/
    function update($module,$module_uid, $auth_data){
        if($module != 'pam_passport_sso' && $module!='pam_passport_basic' && $module != 'pam_passport_token'){
        ...
        $cas_member = $_cas_member -> getRow('*',array('member_id' => $account_id));
        if($cas_member){
            $_SESSION['account']['guid'] = $cas_member['guid'];
        }
