...
final class weixin_refund_plugin_mwxh5 extends ectools_newrefund_app {
     ...
     * @var string 支付方式key
     */
    public $app_key = 'mwxh5';

    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'mwxh5';
    ...
    public function dorefund(&$refundInfo) {
        ...
        if ($refundRes['return_code'] == 'SUCCESS' && $refundRes['result_code'] == 'SUCCESS') {
            ...
            \Neigou\Logger::Debug("mwxh5.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return true;
        } else {
            //默认状态失败
            $refundInfo['status'] = 'failed';
            \Neigou\Logger::General("mwxh5.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return false;
        }
    }
