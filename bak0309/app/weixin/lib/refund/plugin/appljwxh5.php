...
final class weixin_refund_plugin_appljwxh5 extends ectools_newrefund_app {

    /**
     * @var string 支付方式名称
     */
    public $name = 'APP微信支付H5';

    /**
     * @var string 支付方式接口名称
     */
    public $app_name = 'APP微信支付H5';

    /**
     * @var string 支付方式key
     */
    public $app_key = 'appljwxh5';

    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'appljwxh5';

    /**
     * @var string 统一显示的名称
     */
    public $display_name = 'APP微信支付H5';
    ...
    public function dorefund(&$refundInfo) {
        $appId = trim($this->getConf('appId', 'weixin_payment_plugin_appljwxh5')); //appid公众号 ID
        $paySignKey = trim($this->getConf('paySignKey', 'weixin_payment_plugin_appljwxh5')); //PaySignKey 对应亍支付场景中的 appKey 值
        $partnerId = trim($this->getConf('partnerId', 'weixin_payment_plugin_appljwxh5')); //商户ID
        ...
        \Neigou\Logger::General("appljwxh5.beforedorefund", array(
            'app_id' => $appId,
            'paySignKey' => $paySignKey,
            'fields' => $this->fields,
            'weixinCertPath' => $weixinCertPath,
            'apiclient_cert_size' => filesize($weixinCertPath .  '/apiclient_cert.pem'),
            'apiclient_key_size' => filesize($weixinCertPath .  '/apiclient_key.pem'),
        ));
        ...
        if ($refundRes['return_code'] == 'SUCCESS' && $refundRes['result_code'] == 'SUCCESS') {
            ...
            \Neigou\Logger::Debug("appljwxh5.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return true;
        } else {
            //默认状态失败
            $refundInfo['status'] = 'failed';
            \Neigou\Logger::General("appljwxh5.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return false;
        }
    }
