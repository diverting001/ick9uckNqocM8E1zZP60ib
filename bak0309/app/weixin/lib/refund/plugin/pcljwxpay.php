...
final class weixin_refund_plugin_pcljwxpay extends ectools_newrefund_app {
    ...
    public function dorefund(&$refundInfo) {
        $appId = trim($this->getConf('appId', 'weixin_payment_plugin_pcljwxpay')); //appid公众号 ID
        $paySignKey = trim($this->getConf('paySignKey', 'weixin_payment_plugin_pcljwxpay')); //PaySignKey 对应亍支付场景中的 appKey 值
        $partnerId = trim($this->getConf('partnerId', 'weixin_payment_plugin_pcljwxpay')); //商户ID
        ...
        if ($refundRes['return_code'] == 'SUCCESS' && $refundRes['result_code'] == 'SUCCESS') {
            ...
            \Neigou\Logger::Debug("pcljwxpay.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return true;
        } else {
            //默认状态失败
            $refundInfo['status'] = 'failed';
            \Neigou\Logger::General("pcljwxpay.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return false;
        }
    }
