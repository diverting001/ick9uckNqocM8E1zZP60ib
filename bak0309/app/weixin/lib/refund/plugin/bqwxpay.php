...
final class weixin_refund_plugin_bqwxpay extends ectools_newrefund_app {
    ...
    public function dorefund(&$refundInfo) {
        $appId = trim($this->getConf('appId', 'weixin_payment_plugin_bqwxpay')); //appid公众号 ID
        $paySignKey = trim($this->getConf('paySignKey', 'weixin_payment_plugin_bqwxpay')); //PaySignKey 对应亍支付场景中的 appKey 值
        $partnerId = trim($this->getConf('partnerId', 'weixin_payment_plugin_bqwxpay')); //商户ID
        ...
        $weixinCertPath = $this->getConf('cert_path','weixin_payment_plugin_bqwxpay');
        ...
        if ($refundRes['return_code'] == 'SUCCESS' && $refundRes['result_code'] == 'SUCCESS') {
            ...
            \Neigou\Logger::Debug("bqwxpay.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return true;
        } else {
            //默认状态失败
            $refundInfo['status'] = 'failed';
            \Neigou\Logger::General("bqwxpay.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return false;
        }
    }
        ...
        if ($refundRes['return_code'] == 'SUCCESS' && $refundRes['result_code'] == 'SUCCESS' && $refundRes['refund_count'] == 1 && $refundRes['refund_status_0']) {
            ...
            switch ($refundRes['refund_status_0']) {
                case 'SUCCESS':
                    ...
                    $report_name = 'bqwx.getRefundStatus.info';
                    break;
                case 'PROCESSING':
                    $refundInfo['status'] = 'progress';
                    $report_name = 'bqwx.getRefundStatus.info';
                ...
                case 'REFUNDCLOSE':
                    ...
                    $report_name = 'bqwx.getRefundStatus.error';
                    break;
                case 'CHANGE':
                    $refundInfo['status'] = 'abnormal';
                    $report_name = 'bqwx.getRefundStatus.error';
            ...
            if ($report_name == 'bqwx.getRefundStatus.info') {
