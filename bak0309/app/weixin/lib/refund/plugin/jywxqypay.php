...
final class weixin_refund_plugin_jywxqypay extends ectools_newrefund_app {
    ...
    public function dorefund(&$refundInfo) {
        $appId = trim($this->getConf('appId', 'weixin_payment_plugin_jywxqypay')); //appid公众号 ID
        $paySignKey = trim($this->getConf('paySignKey', 'weixin_payment_plugin_jywxqypay')); //PaySignKey 对应亍支付场景中的 appKey 值
        $partnerId = trim($this->getConf('partnerId', 'weixin_payment_plugin_jywxqypay')); //商户ID
        ...
        $weixinCertPath = $this->getConf('cert_path','weixin_payment_plugin_jywxpay');
        ...
        if ($refundRes['return_code'] === 'SUCCESS' && $refundRes['result_code'] === 'SUCCESS') {
            ...
            \Neigou\Logger::Debug("jywxqypay.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return true;
        }

        //默认状态失败
        $refundInfo['status'] = 'failed';
        \Neigou\Logger::General("jywxqypay.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
        return false;
    }
        ...
        if ($refundRes['return_code'] === 'SUCCESS' && $refundRes['result_code'] === 'SUCCESS' && $refundRes['refund_count'] == 1 && $refundRes['refund_status_0']) {
            ...
            switch ($refundRes['refund_status_0']) {
                case 'SUCCESS':
                    ...
                    $report_name = 'jyqywx.getRefundStatus.info';
                    break;
                case 'PROCESSING':
                    $refundInfo['status'] = 'progress';
                    $report_name = 'jyqywx.getRefundStatus.info';
                ...
                default:
                    ...
                    $report_name = 'jyqywx.getRefundStatus.error';
                    break;
            }
            if ($report_name === 'jyqywx.getRefundStatus.info') {
