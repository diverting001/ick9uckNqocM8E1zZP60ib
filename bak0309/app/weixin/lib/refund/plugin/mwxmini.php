...
final class weixin_refund_plugin_mwxmini extends ectools_newrefund_app {
    ...
    public function dorefund(&$refundInfo) {
        $appId = trim($this->getConf('app_id', 'wap_payment_plugin_mwxmini')); //appid公众号 ID
        $paySignKey = trim($this->getConf('pay_sign', 'wap_payment_plugin_mwxmini')); //PaySignKey 对应亍支付场景中的 appKey 值
        $partnerId = trim($this->getConf('mch_id', 'wap_payment_plugin_mwxmini')); //商户ID
        ...
        if ($refundRes['return_code'] == 'SUCCESS' && $refundRes['result_code'] == 'SUCCESS') {
            ...
            \Neigou\Logger::Debug("mwxmini.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return true;
        } else {
            //默认状态失败
            $refundInfo['status'] = 'failed';
            \Neigou\Logger::General("mwxmini.dorefund", array('sparam1' => json_encode($refundInfo), 'sparam2' => json_encode($refundRes)));
            return false;
        }
    }
        ...
        if ($refundRes['return_code'] == 'SUCCESS' && $refundRes['result_code'] == 'SUCCESS' && $refundRes['refund_count'] == 1 && $refundRes['refund_status_0']) {
            ...
            switch ($refundRes['refund_status_0']) {
                case 'SUCCESS':
                    ...
                    $report_name = 'wxmini.getRefundStatus.info';
                    break;
                case 'PROCESSING':
                    $refundInfo['status'] = 'progress';
                    $report_name = 'wxmini.getRefundStatus.info';
                ...
                case 'REFUNDCLOSE':
                    ...
                    $report_name = 'wxmini.getRefundStatus.error';
                    break;
                case 'CHANGE':
                    $refundInfo['status'] = 'abnormal';
                    $report_name = 'wxmini.getRefundStatus.error';
            ...
            if ($report_name == 'wxmini.getRefundStatus.info') {
