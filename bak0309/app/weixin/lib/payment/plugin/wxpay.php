...
final class weixin_payment_plugin_wxpay extends ectools_payment_app implements ectools_interface_payment_app {
    ...
    public function __construct($app){
        parent::__construct($app);
         ...
         $this->notify_url = kernel::openapi_url('openapi.weixin', 'wxpay');
        ...
        return array(
           ...
           /* 'appId'=>array(
            ...
            'get_openid_url'=>array(
                'title'=>app::get('weixin')->_('open_id 获取URL 重定向需要提供给第三方 [site_url]/openapi/ectools_payment/parse/b2c/weixin_payment_plugin_wxpay/openid'),
                'type'=>'string',
            ),
            'pay_type'=>array(
    ...
    public function dopay($payment)
    {
        ...
        if($get_openid_channel == 'juyoufuli'){
            ...
            $_redis -> fetch('payment_wxpay_open_id_',$payment['member_id'],$open_id);
            if(empty($open_id)){
                //先保存当前支付单信息
                $_redis->store('payment_wxpay_info_',$payment['payment_id'],$this->fields,300);
                //开始调用获取openid接口
...
//        $third_company_member_mdl = app::get('b2c') -> model('third_members');
//        $third_member_info = $third_company_member_mdl -> getRow('*',array('internal_id' => $payment['member_id'], 'channel'=>'weixin_qy'));
//        if (!empty($third_member_info)) {
//            // 微信企业号用户使用扫码支付的方式。
...
//        } else {
        {
            ...
            $title = empty($config_title)?'内购网':$config_title;
            ...
            if($payment['is_certification']){
                ...
                \Neigou\Logger::General('ecstore.wxpay.global',array('remark'=>'海外购订单','card_id'=>$payment['card_id'],'card_name'=>$payment['card_name'],'payment'=>$payment));
            }
            $prepay_id = $this->get_prepay_id($appId,$paySignKey);
            if($prepay_id ==''){
                \Neigou\Logger::General('ecstore.wxpay',array('remark'=>'获取prepay_id失败','request_data'=>json_encode($this->fields),'order_id'=>$payment['order_id'],'payment'=>$payment));
    ...
    function openid(){
        ...
        if(empty($open_id)){
            $this->splash('failed', 'back', app::get('b2c')->_('微信授权登陆失败！请联系客服'));
        }
        ...
        if($member_id){
            //跳转到支付
            $_redis = kernel::single('base_sharedkvstore');
            $_redis->store('payment_wxpay_open_id_',$member_id,$open_id,300);
            ...
            $data['def_pay']['pay_app_id'] = 'wxpay';
            $data['def_pay']['cur_money'] = $sdf['money'];
            $query['payment'] = $data;
            //设置订单对应的payment_id 为当前
            $_redis->store('payment_wxpay_payment_id_',$sdf['rel_id'],$payment_id,300);
