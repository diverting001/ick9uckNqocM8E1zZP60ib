<?php
final class weixin_payment_plugin_jywxqypay extends ectools_payment_app implements ectools_interface_payment_app {
    /**
     * @var string 支付方式名称
     */
    public $name = '骏业企业微信支付';
    /**
     * @var string 支付方式接口名称
     */
    public $app_name = '骏业企业微信支付';
    /**
     * @var string 支付方式key
     */
    public $app_key = 'jywxqypay';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'jywxqypay';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '骏业企业微信支付';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.weixin', 'jywxqypay');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/weixin/weixin_payment_plugin_jywxqypay', 'callback');
    ...
    public function setting(){
        ...
        return array(
            ...
            'title'=>array(
                'title'=>app::get('weixin')->_('支付标题title'),
                'type'=>'string',
            ),
            'suiteId'=>array(
                'title'=>app::get('weixin')->_('suiteId'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'partnerId'=>array(
    ...
    public function intro(){
        return app::get('weixin')->_('骏业微信企业支付 2.1.0');
    }

    // 接口请求
    private function request($api = '',$post_data = array()){
        $url = AGH_API_URL . $api;
        $getData = $this -> signData($post_data);
        $url .= '?' . http_build_query($getData);
        $curl = new \Neigou\Curl();
        $curl -> SetHeader('Content-Type','application/x-www-form-urlencoded');
        $result = $curl -> Post($url,$post_data);
        \Neigou\Logger::General("jywxqypay.api",array('data' => $url . "\n" . json_encode($post_data) . "\n" . $result));
        return $result;
    }

    // 签名数据
    private function signData($get_data = array()){
        foreach ($get_data as $k => $v) {
            if(($v == '' || $v == null) && $v !== 0 ) unset($get_data[$k]);
        }
        $get_data['partnerId'] = CAS_PARTNER_ID;
        $get_data['expires'] = $get_data['expires'] ? $get_data['expires'] : time() + 30;
        $get_data['nonce'] = $get_data['nonce'] ? $get_data['nonce'] : md5(microtime() . rand(0,999999));
        ksort($get_data);
        $sign_arr = array();
        ...
        $signature = hash_hmac('sha256',$sign_str,CAS_KEY);
    ...
    // 供外部调用
    public function get_access_token($qy_corpid){
        if(!$qy_corpid) return '';
        $api = '/fe/private/';
        $data = array();
        $data['qy_corpid'] = $qy_corpid;
        $data['name'] = 'corp.get_access_token';
        $data['version'] = 'v1';
    ...
    public function dopay($payment)
    {
        ...
        $suiteId      = trim($this->getConf('suiteId',      __CLASS__));

        $_order_model = app::get('b2c') -> model('orders');
        //$order_info = $_order_model -> getRow('*',array('order_id' => $payment['order_id']));
        $order_info = kernel::single("b2c_service_order")->getOrderInfo($payment['order_id']);
        $company_id = $order_info['company_id'];
        $member_id = $order_info['member_id'];

        $guid = kernel::single('b2c_member') -> _cas -> getGuidByMemberId($member_id);
        $gcorp = app::get('b2c') -> model('club_cas_company') -> getCasCompany($company_id);
        $gcorp_id = $gcorp['gcorp_id'];

        if(!$guid){
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("jywxwork.pay.fatal.error", array('request' => $payment, 'cookie' => $_COOKIE,'msg' => '用户状态异常，请联系客服处理[guid]'));
            die('用户状态异常，请联系客服处理[guid]');
        }
        if(!$gcorp_id){
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("jywxwork.pay.fatal.error", array('request' => $payment, 'cookie' => $_COOKIE,'msg' => '用户状态异常，请联系客服处理[gcorp_id]'));
            die('用户状态异常，请联系客服处理[gcorp_id]');
        }

        $openid = $payment['wxopenid'];

        if(!$openid){
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("jywxwork.pay.fatal.error", array('request' => $payment, 'cookie' => $_COOKIE,'msg' => '用户状态异常，请联系客服处理[openid]'));
            die('用户状态异常，请联系客服处理[openid]');
        }

        $this->add_field("bank_type"          , "WX" );
        $this->add_field("body"               , strval( str_replace(' ', '', (isset($payment['body']) && $payment['body']) ? $payment['body'] : app::get('weixin')->_('订单支付') ) ) );
        ...
        $this->add_field('openid'             ,strval( $openid ));
        //海外购订单增加实名信息
        if($payment['is_global']){
            $this->add_field('user_creid',$payment['card_id']);
            $this->add_field('user_truename',$payment['card_name']);
            \Neigou\Logger::General('ecstore.jywxqypay.global',array('remark'=>'海外购订单','card_id'=>$payment['card_id'],'card_name'=>$payment['card_name'],'payment'=>$payment));
        }
        $prepay_id = $this->get_prepay_id($appId,$paySignKey);
        if($prepay_id ==''){
            \Neigou\Logger::General('ecstore.jywxqypay',array('remark'=>'获取prepay_id失败','request_data'=>json_encode($this->fields),'order_id'=>$payment['order_id'],'payment'=>$payment));
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("jywxwork.pay.fatal.error", array('request' => $payment, 'cookie' => $_COOKIE,'msg' => '获取统一支付prepay_id错误'));
            die("获取统一支付prepay_id错误");
        }
        $this->add_field('prepay_id'          ,$prepay_id);
        $this->add_field("create_biz_package" , $this->create_biz_package($appId, $paySignKey, $partnerId) );

        // 用于微信支付后跳转页面传order_id,不作为传微信的字段
        $this->add_field("order_id"      , $payment['order_id'] );

        $this->add_field("order_info"      , $order_info);

        $scheme = kernel::single('base_component_request')->get_server('HTTPS')=='on'?'HTTPS':'HTTP';
        $scheme = strtolower($scheme);
        $host = isset($_SERVER['HTTP_X_FORWARDED_HOST']) ? $_SERVER['HTTP_X_FORWARDED_HOST'] : (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '');
        $current_url = $scheme . '://'.$host.$_SERVER['REQUEST_URI'];
        $current_url = explode('#',$current_url);
        $current_url = current($current_url);

        // 生成jssdk.config,不作为传微信的字段
        $this -> add_field("config"             ,weixin_commonUtil::getJsapiConfigFromAgh($current_url, $appId, $suiteId));
        $this -> add_field("appid"             ,$appId);

        \Neigou\Logger::General("jywxqy.dopay", array('data' => $this->fields));
    ...
    protected function get_html(){
        ...
        $success_url = app::get('wap')->router()->gen_url(array('app'=> $app,'ctl'=>'wap_paycenter2','act'=>'result_wait','full'=>1,'arg0'=>$this->fields['order_id'],'arg1'=>'true'));
        $failure_url = app::get('wap')->router()->gen_url(array('app'=> $app,'ctl'=>'wap_paycenter2','act'=>'result_failed','full'=>1,'arg0'=>$this->fields['order_id'],'arg1'=>'result_placeholder'));
        $html = <<<START
<!DOCTYPE html>
<html>
<head>
...
<script type="text/javascript">
    wx.config({
        debug: false,
        appId: "{$this -> fields['config']['appId']}",
        timestamp: "{$this -> fields['config']['timestamp']}",
        nonceStr: "{$this -> fields['config']['nonceStr']}",
        signature: "{$this -> fields['config']['signature']}",
        jsApiList: [
    ...
    function callpay(){
        WeixinJSBridge.invoke(
            "getBrandWCPayRequest",
            {$this->fields["create_biz_package"]},
            function(res){
...
//↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓公共函数部分↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
    ...
    private function check_cft_parameters() {
        if($this->fields["bank_type"] == null || $this->fields["body"] == null || $this->fields["mch_id"] == null ||
    ...
    private function get_cft_package($paySignKey){
        return 'prepay_id='.$this->fields['prepay_id'];
        try {
            if (null == $paySignKey || "" == $paySignKey ) {
                throw new Exception("支付密钥不能为空！" . "<br>");
            }
            ...
            return $paraString . "&sign=" . weixin_commonUtil::sign($unSignParaString,weixin_commonUtil::trimString($paySignKey));
        } catch (Exception $e) {
            die($e->getMessage());
        }
...
//↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑公共函数部分↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
    ...
    private function qr_paymemt($payment) {
        ...
        $this->add_field("bank_type"          , "WX" );
        $this->add_field("body"               , strval( str_replace(' ', '', (isset($payment['body']) && $payment['body']) ? $payment['body'] : app::get('weixin')->_('内购网订单') ) ) );
        $this->add_field("mch_id"             , strval( $partnerId ) );
        $this->add_field("out_trade_no"       , strval( $payment['payment_id'] ) );
        ...
        $this->add_field('openid'             ,strval( $payment['wxopenid'] ));
        $prepay_info = weixin_commonUtil::getPrepayInfo($appId,$paySignKey,$this->fields);
        \Neigou\Logger::General("wxpay.qr_paymemt", array("action"=>"getPrepayInfo", "weixinfields" => json_encode($this->fields),"prepay_info" => json_encode($prepay_info)));
        if (empty($prepay_info['code_url'])) {
            die("获取统一支付code_url错误");
        }
        $qr_pay_url = app::get('wap')->router()->gen_url(array('app'=>'weixin','ctl'=>'wap_wxin','act'=>'qr_pay',
    ...
    public function getQyWxDataCallback()
    {
        ...
        if (!$appid) {
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("jywxwork.pay.fatal.error",
                array('request' => $_REQUEST, 'cookie' => $_COOKIE, 'msg' => '支付方式错误[appid]'));
            die('支付方式错误[appid]');
        }
        ...
        if (!$access_token) {
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("jywxwork.pay.fatal.error",
                array('request' => $_REQUEST, 'cookie' => $_COOKIE, 'msg' => '获取用户授权失败[access_token]'));
            die('获取用户授权失败[access_token]');
        }

        if (!$code) {
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("jywxwork.pay.fatal.error",
                array('request' => $_REQUEST, 'cookie' => $_COOKIE, 'msg' => '支付授权失败[code]'));
            die('支付授权失败[code]');
        }

        $openid = weixin_commonUtil::getQyWeixinOpenId($access_token, $code);
        if (!$openid) {
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("jywxwork.pay.fatal.error",
                array('request' => $_REQUEST, 'cookie' => $_COOKIE, 'msg' => '用户授权失败[openid]'));
            die('用户授权失败[openid]');
        }

        $_SESSION['wxqy_data'] = array('openid' => $openid, 'access_token' => $access_token);
    ...
    public function getQyWxData($callurl){
        $appid = trim($this->getConf('appId', __CLASS__)); // appid 公众号 ID
        if(!$appid){
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("jywxwork.pay.fatal.error", array('request' => $_REQUEST, 'cookie' => $_COOKIE,'msg' => '支付方式错误[appid]'));
            die('支付方式错误[appid]');
        }
        $callurl = base64_encode($callurl);
        $key = md5(microtime().microtime().microtime());
        $redirect_uri =  urlencode(
            kernel::openapi_url('openapi.ectools_payment/parse/weixin/weixin_payment_plugin_jywxqypay', 'getQyWxDataCallback').'?url='. $callurl);
        $cas_call_url = AGH_CAS_REDIRECT_GETWAY . '?redirect_url=' . $redirect_uri;
        $cas_call_url = urlencode($cas_call_url);
        $url = sprintf('https://open.weixin.qq.com/connect/oauth2/authorize?appid=%s&redirect_uri=%s&response_type=code&scope=snsapi_base&state=%s#wechat_redirect',$appid,$cas_call_url,$key);
