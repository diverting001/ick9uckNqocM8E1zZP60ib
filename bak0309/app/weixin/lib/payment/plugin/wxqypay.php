<?php
final class weixin_payment_plugin_wxqypay extends ectools_payment_app implements ectools_interface_payment_app {
    /**
     * @var string 支付方式名称
     */
    public $name = '企业微信支付';
     ...
     * @var string 支付方式接口名称
     */
    public $app_name = '企业微信支付';
     /**
     * @var string 支付方式key
     */
    public $app_key = 'wxqypay';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'wxqypay';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '企业微信支付';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.weixin', 'wxqypay');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/weixin/weixin_payment_plugin_wxqypay', 'callback');
    ...
    public function intro(){
        return app::get('weixin')->_('微信企业支付 2.1.0');
    }
    ...
    private function request($api = '',$post_data = array()){
        ...
        \Neigou\Logger::General("wxqypay.api",array('data' => $url . "\n" . json_encode($post_data) . "\n" . $result));
        return $result;
    }
    ...
    public function dopay($payment)
    {
        ...
        if(!$guid){
            ...
            \Neigou\Logger::General("wxwork.pay.fatal.error", array('request' => $payment, 'cookie' => $_COOKIE,'msg' => '用户状态异常，请联系客服处理[guid]'));
            die('用户状态异常，请联系客服处理[guid]');
        }
        if(!$gcorp_id){
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("wxwork.pay.fatal.error", array('request' => $payment, 'cookie' => $_COOKIE,'msg' => '用户状态异常，请联系客服处理[gcorp_id]'));
...
//        if(!$access_token){
//            header("Content-Type: text/html;charset=UTF-8");
//            \Neigou\Logger::General("wxwork.pay.fatal.error", array('request' => $payment, 'cookie' => $_COOKIE,'msg' => '用户状态异常，请联系客服处理[access_token]'));
//            die('用户状态异常，请联系客服处理[access_token]');
//        }

        if(!$openid){
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("wxwork.pay.fatal.error", array('request' => $payment, 'cookie' => $_COOKIE,'msg' => '用户状态异常，请联系客服处理[openid]'));
        ...
        if($prepay_id ==''){
            header("Content-Type: text/html;charset=UTF-8");
            \Neigou\Logger::General("wxwork.pay.fatal.error", array('request' => $payment, 'cookie' => $_COOKIE,'msg' => '获取统一支付prepay_id错误'));
        ...
        $this -> add_field("config"             ,weixin_commonUtil::getJsapiConfigFromAgh($current_url, $appId));
        $this -> add_field("appid"             ,$appId);

        \Neigou\Logger::General("wxqy.dopay", array('data' => $this->fields));
    ...
    protected function get_html(){
       ...
       header("Content-Type: text/html;charset=".$this->submit_charset);
        $app = 'b2c';
        $title = '内购网';
        if($this -> fields['order_info']['platform'] == 2){
            $app = 'jifen';
            $title = '积分宝';
        }
...
return $html;
/*
        $order_info = app::get('b2c') -> model('orders') -> getRow('*',array('order_id' => $this->fields['order_id']));
        $app = 'b2c';
        $title = '内购网';
        if($order_info['platform'] == 2){
            $app = 'jifen';
            $title = '积分宝';
        }

        $success_url = app::get('wap')->router()->gen_url(array('app'=> $app,'ctl'=>'wap_paycenter','act'=>'result_pay','full'=>1,'arg0'=>$this->fields['order_id'],'arg1'=>'true'));
        $failure_url = app::get('wap')->router()->gen_url(array('app'=> $app,'ctl'=>'wap_paycenter','act'=>'result_failed','full'=>1,'arg0'=>$this->fields['order_id'],'arg1'=>'result_placeholder'));
        $strHtml = '<html>
