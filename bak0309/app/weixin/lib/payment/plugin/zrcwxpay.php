<?php
final class weixin_payment_plugin_zrcwxpay extends ectools_payment_app implements ectools_interface_payment_app {

    /**
     * @var string 支付方式名称
     */
    public $name = '自然商城微信支付';

    /**
     * @var string 支付方式接口名称
     */
    public $app_name = '自然商城微信支付';

     /**
     * @var string 支付方式key
     */
    public $app_key = 'zrcwxpay';

    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'zrcwxpay';

    /**
     * @var string 统一显示的名称
     */
    public $display_name = '自然商城微信支付';
    ...
    public function __construct($app)
    {
        ...
        $this->notify_url = kernel::openapi_url('openapi.weixin', 'zrcwxpay');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/weixin/weixin_payment_plugin_zrcwxpay', 'callback');
    ...
    public function setting()
    {
        ...
        return array(
           ...
           /* 'appId' => array(
            ...
            'get_openid_channel'=>array(
                'title'=>app::get('weixin')->_('open_id 获取渠道 针对juyoufuli neigou cofco'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'get_openid_url'=>array(
                'title'=>app::get('weixin')->_('open_id 获取URL 重定向需要提供回调地址 [?callbackURL]'),
                'type'=>'string',
            ),
            'pay_type' => array(
    ...
    public function dopay($payment)
    {
        ...
        $get_openid_channel = $this->getConf('get_openid_channel', __CLASS__);
        $open_id = '';
        if ($get_openid_channel == 'juyoufuli') {
            // 需要走聚优福利的获取openid方式
            // 1.查询是否是跳转回来的请求
            $_redis -> fetch('payment_zrcwxpay_open_id_', $payment['member_id'], $open_id);
            if (empty($open_id)) {
                // 先保存当前支付单信息
                $_redis->store('payment_zrcwxpay_info_', $payment['payment_id'], $this->fields, 300);
                // 开始调用获取openid接口
                $url = $this->getConf('get_openid_url', __CLASS__);
                $url .= '&key='.$payment['payment_id'];
                header('Location:'.$url);
                die;
            }
        }
        ...
        $config_title = $this->getConf('title', __CLASS__);
        $title = empty($config_title) ? '聚优网' : $config_title;

        $this->add_field("bank_type", "WX");
        $this->add_field("body", strval(str_replace(' ', '', (isset($payment['body']) && $payment['body']) ? $payment['body'] : $title.'订单')));
        $this->add_field("mch_id", strval($partnerId));
        $this->add_field("out_trade_no", strval( $payment['payment_id']));
        $this->add_field("total_fee", intval(bcmul($payment['cur_money'], 100))); // @TODO maojz 使用高精度乘法函数替换
        $this->add_field("fee_type", $this->curname);
        ...
        $this->add_field('openid', empty($open_id) ? strval($payment['wxopenid']) : strval($open_id));
        ...
        if ($payment['is_certification']) {
            ...
            \Neigou\Logger::General('ecstore.zrcwxpay.global', array(
                'remark'    => '海外购订单',
        ...
        if ($prepay_id == '') {
            \Neigou\Logger::General('ecstore.zrcwxpay', array(
    ...
    public function openid()
    {
        ...
        if (empty($open_id)) {
            ...
            $c->splash('failed', 'back', app::get('b2c')->_('微信授权登陆失败！请联系客服'));
        }
        ...
        if ($member_id) {
            // 跳转到支付
            $_redis = kernel::single('base_sharedkvstore');
            $_redis->store('payment_zrcwxpay_open_id_', $member_id, $open_id, 300);
            $payment_id = trim($_REQUEST['key']);
            ...
            $data['def_pay']['pay_app_id'] = 'zrcwxpay';
            $data['def_pay']['cur_money'] = $sdf['money'];
            $query['payment'] = $data;

            // 设置订单对应的payment_id 为当前
            $_redis->store('payment_zrcwxpay_payment_id_', $sdf['rel_id'], $payment_id, 300);
            $r = app::get('wap')->router()->gen_url(array('app' => 'b2c', 'ctl' => 'wap_paycenter2', 'act' => 'dopayment', 'arg0' => 'pop_order'));

            \Neigou\Logger::General('ecstore.zrcwxpay.openid', array(
                'action' => 'openid_zrcwxpay.v',
                'member' => $member,
                'sdf'    => $sdf,
                'data'   => $data,
                'query'  => $query,
