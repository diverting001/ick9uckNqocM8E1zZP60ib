<?php
final class weixin_payment_plugin_pcljwxpay extends ectools_payment_app implements ectools_interface_payment_app {
     ...
     * @var string 支付方式key
     */
    public $app_key = 'pcljwxpay';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'pcljwxpay';
    /**
     * @var string 统一显示的名称
     */
    ...
    public function __construct($app){
        parent::__construct($app);
         ...
         $this->notify_url = kernel::openapi_url('openapi.weixin', 'pcljwxpay');
    ...
    private function qr_paymemt($payment) {
        ...
        $body = strval( str_replace(' ', '', (isset($payment['body']) && $payment['body']) ? $payment['body'] : $config_title.'订单' ) ) ;
        ...
        if($payment['is_certification']){
            ...
            \Neigou\Logger::General('ecstore.pcljwxpay.global',array('remark'=>'海外购订单','card_id'=>$payment['card_id'],'card_name'=>$payment['card_name'],'payment'=>$payment));
        }
        $prepay_info = weixin_commonUtil::getPrepayInfo($appId,$paySignKey,$this->fields);
        \Neigou\Logger::General("pcljwxpay.qr_paymemt", array("action"=>"getPrepayInfo", "weixinfields" => json_encode($this->fields),"prepay_info" => json_encode($prepay_info)));
        if (empty($prepay_info['code_url'])) {
            \Neigou\Logger::General('ecstore.pcljwxpay',array('remark'=>'获取code_url失败','request_data'=>json_encode($this->fields),'order_id'=>$payment['order_id'],'payment'=>$payment));
