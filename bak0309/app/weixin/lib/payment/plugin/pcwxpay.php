<?php
final class weixin_payment_plugin_pcwxpay extends ectools_payment_app implements ectools_interface_payment_app {
     ...
     * @var string 支付方式key
     */
    public $app_key = 'pcwxpay';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'pcwxpay';
    /**
     * @var string 统一显示的名称
     */
    ...
    public function __construct($app){
        parent::__construct($app);
         ...
         $this->notify_url = kernel::openapi_url('openapi.weixin', 'pcwxpay');
    ...
    private function qr_paymemt($payment) {
        ...
        if($payment['is_certification']){
            ...
            \Neigou\Logger::General('ecstore.pcwxpay.global',array('remark'=>'海外购订单','card_id'=>$payment['card_id'],'card_name'=>$payment['card_name'],'payment'=>$payment));
        }
        ...
        if (empty($prepay_info['code_url'])) {
            \Neigou\Logger::General('ecstore.pcwxpay',array('remark'=>'获取code_url失败','request_data'=>json_encode($this->fields),'order_id'=>$payment['order_id'],'payment'=>$payment));
