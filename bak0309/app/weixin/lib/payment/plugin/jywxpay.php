<?php
final class weixin_payment_plugin_jywxpay extends ectools_payment_app implements ectools_interface_payment_app {
     ...
     * @var string 支付方式接口名称
     */
    public $app_name = '骏业微信支付';
     /**
     * @var string 支付方式key
     */
    public $app_key = 'jywxpay';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'jywxpay';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '骏业微信支付';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    public function __construct($app){
        parent::__construct($app);
         ...
         $this->notify_url = kernel::openapi_url('openapi.weixin', 'jywxpay');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/weixin/weixin_payment_plugin_jywxpay', 'callback');
    ...
    public function intro(){
        return app::get('weixin')->_('骏业微信支付是由腾讯公司知名移动社交通讯软件微信及第三方支付平台财付通联合推出的移动支付创新产品，旨在为广大微信用户及商户提供更优质的支付服务，微信的支付和安全系统由腾讯财付通提供支持。财付通是持有互联网支付牌照并具备完备的安全体系的第三方支付平台。');
    }
    ...
    public function dopay($payment)
    {
        ...
        $this->add_field("body", (string)str_replace(' ', '',
            (isset($payment['body']) && $payment['body']) ? $payment['body'] : '骏业科技订单'));
        ...
        $_redis -> fetch('payment_jywxpay_open_id_',$payment['member_id'],$open_id);
        if(empty($open_id)){
            //先保存当前支付单信息
            $_redis->store('payment_jywxpay_info_',$payment['payment_id'],$this->fields,300);
            //开始调用获取openid接口

            $url = kernel::openapi_url('openapi.ectools_payment/parse/weixin/weixin_payment_plugin_jywxpay', 'getOpenId');
            $url .= '?payment_id='.$payment['payment_id'];
            header('Location:'.$url);
            die;
        }
        $this->add_field('openid', (string)$open_id);
        //添加身份验证参数
        //判断订单是否是海外购订单
        if ($payment['is_certification']) {
            $this->add_field('user_creid', $payment['card_id']);
            $this->add_field('user_truename', $payment['card_name']);
            \Neigou\Logger::General('ecstore.wxpay.global', array(
                'remark' => '海外购订单',
        ...
        if ($prepay_id == '') {
            \Neigou\Logger::General('ecstore.wxpay', array(
                'remark' => '获取prepay_id失败',
                'request_data' => json_encode($this->fields),
    ...
    public function getOpenId(){
        $tourl = kernel::openapi_url('openapi.ectools_payment/parse/weixin/weixin_payment_plugin_jywxpay', 'getWxDataCallback');
    ...
    public function getWxDataCallback(){
        ...
        $data = array();
        $data['appid'] = $this->getConf('appId',__CLASS__);
        $data['secret'] = $this->getConf('appSecret',__CLASS__);
        ...
        if($res){
            ...
            if($result['openid']){
                ...
                if($member_id){
                    ...
                    $_redis->store('payment_jywxpay_open_id_',$member_id,$result['openid'],300);
                    ...
                    $data['def_pay']['pay_app_id'] = 'jywxpay';
                    $data['def_pay']['cur_money'] = $sdf['money'];
                    $query['payment'] = $data;
                    //设置订单对应的payment_id 为当前
                    $_redis->store('payment_jywxpay_payment_id_',$sdf['rel_id'],$payment_id,300);
                    $r = app::get('wap')->router()->gen_url(array('app'=>'b2c','ctl'=>'wap_paycenter2','act'=>'dopayment','arg0'=>'pop_order'));
//
                    header('Location: '.$r.'?'.http_build_query($query));

                    exit;
                }
                $c = new wap_controller();
                $c->splash('failed', 'back', app::get('b2c')->_('没有获取到登录账号！'));
            }else{
                $c = new wap_controller();
                $c->splash('failed', 'back', app::get('b2c')->_('获取授权失败！'));
            }
        }else{
            $c = new wap_controller();
            $c->splash('failed', 'back', app::get('b2c')->_('获取微信授权失败！'));
        }
    }
