...
function setHeightAuto(rows,cols){
    for (i = 0, j = rows.length / cols; i < j; i++) {
        var items = rows.slice(i * cols, cols * (i + 1));
        if(items.length) { setEqualHeight(items); }
    }
}
...
function updateCartInfo(data) {
    var number = parseInt(Memory.get('cart_number'));
    var count = parseInt(Memory.get('cart_count'));
    var price = Memory.get('cart_total_price');
    try{

        if(number>=0){
            $$('.op-cart-number').set('text', number);
            $$('.top-mini-cart-txt').set('text', number);
        }

        if(count>0) $$('.op-cart-count').set('text', count);
        if(price || price === 0) $$('.op-cart-total').set('text', price);
    }catch(e){}
}

function updateLike(data){
    var like = parseInt(Memory.get('like_count'));
    try{
        if (like >= 0) {
            var like_num = like;
        }else{
        ...
        $$('.header-like-text').set('html',like_num);
    }catch(e){ }
}
