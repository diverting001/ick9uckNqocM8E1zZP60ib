...
class wap_router implements base_interface_router
{
     ...
     *   @return void
     */
    private function check_https($query) {
        $query_info = $this->parse_query($query);
        $server = kernel::request()->get_host();
        //$this->get_current_sitemap('use_ssl') === false
        if ($_SERVER['HTTP_X_FORWARDED_PORT'] == '8080' && kernel::single('site_route_sslwhitelist')->ChekSSl($query_info['query']) === true) {
            header("Location:https://" . $server . $_SERVER['REQUEST_URI']);
            exit();
        } else if ($_SERVER['HTTP_X_FORWARDED_PORT'] == '8081' && kernel::single('site_route_sslwhitelist')->ChekSSl($query_info['query']) === false) {
            //去掉https强转http逻辑
