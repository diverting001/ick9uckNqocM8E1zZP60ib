...
class wap_controller extends base_controller
{
    ...
    private $exclude_css_by_template = array(
        'wap/product/shopdeals.html',
        'wap/product/shopdetail.html',
        'wap/product/deals.html',
        'wap/product/index.html'
    );
    ...
    /*
        ...
        * @return void
        */
    public function replace_custom_html_label(&$body)
    {
        preg_match_all('/<\s*xstyle.*?>(.*?)<\s*\/\s*xstyle\s*>/is', $body, $matchs);
        if(isset($matchs[0][0]) && !empty($matchs[0][0])){
            foreach($matchs[0] AS $matchcontent){
                $matchcontent_replace = preg_replace('/<\s*xstyle.*?>/','<style>',$matchcontent);
                $matchcontent_replace = preg_replace('/<\s*\/\s*xstyle\s*>/','</style>',$matchcontent_replace);

                $body = str_replace($matchcontent, $matchcontent_replace, $body);
            }
        }
    }//End Function
    ...
    final public function page($view, $no_theme=false, $app_id=null)
    {
        ...
        if (!$params['response_type']){
            ...
            if ( !in_array( $view, $this->exclude_css_by_template ) )
            {
                $this->extract_widgets_css( $html );
            }
            
            $html = str_replace('temp_style', 'style', $html);
            ...
            // 自定义标签解析;by licong
            $this->replace_custom_html_label($html);
     ...
     * @return void
     */
    function splash($status='success', $jumpto=null, $msg=null, $show_notice=false, $wait=0, $ajax=false ,$data=null,$jsonp = ''){
        if($ajax){
     ...
     * @return [type]
     */
    public function ajax_return($code = 0,$data = array(),$msg = '',$callback = ''){
        ob_clean();
        $json = json_encode(array('errno' => $code,'error' => $msg,'body' => $data));
        if($callback){
            echo $callback . "({$json})";
            exit;
        }
     ...
     * @return void
     */
    public function splash_($status='success', $url='', $msg=null, $ajax=false, $data=null){
        if($status == 'failed' || $status == 'error'){
            $status = 'error';
        }else{
            $status='success';
