 ...
 */
class wap_payment_plugin_mfubaokang_server extends ectools_payment_app {
    ...
    public function callback(&$recv) {
        ...
        \Neigou\Logger::General('ecstore.notify.mfubaokang',array('remark'=>'notify_param','data'=>$recv));
        #键名与pay_setting中设置的一致
        if($this->is_return_vaild($recv)){
            $ret['payment_id'] = $recv['out_trade_no'];
            $ret['account'] = $recv['account'];
            $ret['bank'] = app::get('ectools')->_('福保康支付');
            ...
            echo 'fail';
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.notify.mfubaokang',array('remark'=>'sign err','data'=>$recv));
        }
        return $ret;
    }
    ...
    public function is_return_vaild($param) {
        $sign = $param['sign'];
        unset($param['sign']);
        $param['salt'] = $this->getConf('salt','wap_payment_plugin_mfubaokang');
    ...
    public function genSign($data){
        ...
        \Neigou\Logger::General('ecstore.mfubaokang.notify.linkStr',array('action'=>'make sign','linkStr'=>$linkStr,'sign'=>md5($linkStr)));
        return md5($linkStr);
    }
