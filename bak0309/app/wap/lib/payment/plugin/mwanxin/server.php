 ...
 */
class wap_payment_plugin_mwanxin_server extends ectools_payment_app {
    ...
    public function callback(&$recv) {
        ...
        if($this->is_return_vaild($recv)){
            ...
            echo 'fail';
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.notify.mwanxin',array('remark'=>'sign err','data'=>$recv));
        }
        return $ret;
    }
    ...
    public function is_return_vaild($param) {
        $sign = $param['sign'];
        unset($param['sign']);
        $param['salt'] = $this->getConf('salt','wap_payment_plugin_mwanxin');
    ...
    public function genSign($data){
        ...
        \Neigou\Logger::General('ecstore.mwanxin.notify.linkStr',array('action'=>'make sign','linkStr'=>$linkStr,'sign'=>md5($linkStr)));
        return md5($linkStr);
    }
