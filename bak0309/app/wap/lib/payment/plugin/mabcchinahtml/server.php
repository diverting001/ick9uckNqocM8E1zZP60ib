 ...
 * Time: 10:31
 */
class wap_payment_plugin_mabcchinahtml_server extends ectools_payment_app {
    ...
    public function callback(&$recv) {
        ...
        \Neigou\Logger::General('notify.mabcchinahtml.req', array('remark' => '异步通知开始', 'post_data'=>$recv));
        $aMessage = $recv['MSG'];
        //1、还原经过base64编码的信息
        \Neigou\Logger::General('notify.mabcchinahtml.req', array('remark' => '异步通知aMessage', 'post_data'=>$aMessage));
        $tMessage = base64_decode(trim($aMessage));
        //2、取得经过签名验证的报文
        \Neigou\Logger::General('notify.mabcchinahtml.req', array('remark' => '解析base64', 'post_data'=>iconv('GB2312','UTF-8',$tMessage)));
        $mer_id = $this->getConf('mer_id', 'wap_payment_plugin_mabcchinahtml');
        if($this->is_return_vaild($tMessage)){
            \Neigou\Logger::General('notify.mabcchinahtml.req', array('remark' => '交易签名认证通过', 'post_data'=>1));
            $ret['payment_id'] = $this->getValueXml('OrderNo',$tMessage);
            \Neigou\Logger::General('notify.mabcchinahtml.tmp', array('remark' => '交易细节', 'payment_id'=>$ret['payment_id']));
            $ret['account'] = $mer_id;
            $ret['bank'] = app::get('ectools')->_('农行H5支付');
            ...
            $ret['status'] = 'succ';
//            echo 'succ';
            $URL = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mabcchinahtml', 'callback');
            $URL .= '?sn='.$ret['payment_id'];
            print ("<html><head><meta http-equiv=\"refresh\" content=\"0; url='{$URL}'\"></head></html>");
            \Neigou\Logger::General('ecstore.notify.mabcchinahtml', array('remark' => 'trade_succ', 'data' => $ret));
        }else{
            echo 'sign_err';
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.notify.mabcchinahtml',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    ...
    public function query(){
        ...
        \Neigou\Logger::General('ecstore.notify.mabcchinahtml',array('remark'=>'请求参数','field'=>json_encode($field),'tSignature'=>$tSignature,'response_data'=>$res));
    ...
    private function composeRequestMessage($aMessage) {
        $tMessage = "{\"Version\":\"V3.0.0\",\"Format\":\"JSON\",\"Merchant\":" . "{\"ECMerchantType\":\"" . "EBUS" . "\",\"MerchantID\":\"" . $this->getConf('mer_id','wap_payment_plugin_mabcchinahtml') . "\"}," . "\"TrxRequest\":" . $aMessage . "}";
        return $tMessage;
    }

    public function signMessage($aMessage) {
        //1、读取商户证书
        $tMerchantCertFile = $this->getConf('pfx_file','wap_payment_plugin_mabcchinahtml');
//        echo $tMerchantCertFile;die;
        $tMerchantCertPassword = $this->getConf('pfx_file_pass','wap_payment_plugin_mabcchinahtml');
        //2、读取证书
...
//        echo $tRequestMessage;die;
    ...
    public function is_return_vaild($param) {
        ...
        $iTrustpayCertificate = openssl_x509_read($this->der2pem(file_get_contents($this->getConf('trustPay_file','wap_payment_plugin_mabcchinahtml'))));
