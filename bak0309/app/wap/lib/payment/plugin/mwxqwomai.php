 ...
 * Time: 11:12
 */
final class wap_payment_plugin_mwxqwomai extends ectools_payment_app implements ectools_interface_payment_app
{
    /**
     * @var string 支付方式名称
     */
    public $name = '中粮 专用 微信支付';
    /**
     * @var string 支付方式接口名称
     */
    public $app_name = '中粮 专用 微信支付';
    /**
     * @var string 支付方式key
     */
    public $app_key = 'mwxqwomai';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'mwxqwomai';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '中粮 专用 微信支付';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mwxqwomai_server', 'callback');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mwxqwomai', 'callback');
    ...
    public function dopay($payment) {
        ...
        $temp_id = '';
        //1.查询是否是跳转回来的请求
        $_redis->fetch('payment_mwxqwomai_payment_id_',$payment['order_id'],$temp_id);
        if($temp_id){
            $payment['payment_id'] = $temp_id;
            $payment_info = array();
            $_redis->fetch('payment_mwxqwomai_info_',$payment['payment_id'],$payment_info);
        ...
        $this->add_field('orderSource', 'qwomai');//渠道标识
        $this->add_field('nonceStr', $time);//订单JSON
        $this->add_field('outTradeNo', 'QYDD'.$payment['payment_id']);//交易订单编号
        $this->add_field('wxType', 'PUB');//微信支付的类型
        $this->add_field('timeStamp', $time);//交易时间 TODO确认时间戳格式

        //2.获取openid 如果不存在 跳转获取
        $open_id = '';
        $_redis -> fetch('payment_mwxqwomai_open_id_',$payment['payment_id'],$open_id);
        if(empty($open_id)){
            //先保存当前支付单信息
            $_redis->store('payment_mwxqwomai_info_',$payment['payment_id'],$this->fields,300);
            //开始调用获取openid接口
            ...
            if(isset($response['data']['appid']) && $response['msg'] = '成功'){
                ...
                \Neigou\Logger::General('ecstore.mwxqwomai.do_pay', array('action' => 'pay_fail', 'result' => $response));
                header('Location: '.$url);die;
            }
            die;
        }
    ...
    public function genSign($data){
        ...
        \Neigou\Logger::General('ecstore.mwxqwomai.linkStr',array('action'=>'make sign','linkStr'=>$linkStr,'sign'=>md5($linkStr)));
    ...
    public function request($api = '', $post_data = array()) {
        ...
        \Neigou\Logger::General('ecstore.mwxqwomai.req', array('action' => 'do req','req_url'=>$api,'post_data'=>$post_data,'response_data'=>$result));
    ...
    public function callback(&$recv) {
        if($this->is_return_vaild($recv)){
            ...
            $ret['bank'] = app::get('ectools')->_('中粮支付');
            ...
            $ret['pay_app_id'] = "mwxqwomai";
            $ret['pay_type'] = 'online';
            if($recv['payStatus']==2){
                $ret['status'] = 'succ';
                \Neigou\Logger::General('ecstore.callback.mwxqwomai', array('remark' => 'trade_succ', 'data' => $ret));
            } else {
                $ret['status'] = 'invalid';
                \Neigou\Logger::General('ecstore.callback.mwxqwomai',array('remark'=>'trade_err','data'=>$recv));
            }

        }else{
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.callback.mwxqwomai',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    }
