 ...
 * Time: 11:25
 */
final class wap_payment_plugin_mminshengyixiang extends ectools_payment_app implements ectools_interface_payment_app
{
    /**
     * @var string 支付方式名称
     */
     ...
     * @var string 支付方式接口名称
     */
    public $app_name = '民生易享支付';
    /**
     * @var string 支付方式key
     */
    public $app_key = 'mminshengyixiang';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'mminshengyixiang';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '民生易享支付';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    /**
     * wap_payment_plugin_mminshengyixiang constructor.
     * @param $app
     */
    public function __construct($app){
        parent::__construct($app);
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mminshengyixiang_server', 'callback');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mminshengyixiang', 'callback');
    ...
    public function intro(){
        return app::get('ectools')->_('民生易享支付');
    }
    ...
    public function dopay($payment) {
        ...
        $this->add_field('module','NEIGOU');//固定值：ALLINPAY
        ...
        \Neigou\Logger::General('pay.mminshengyixiang',array('remark'=>'支付参数','request_data'=>$this->fields,'des_field'=>$log));
        header('Location:'.$url);
        exit;
    }

    /**
     * 转换 解决对方对于 base64 +=的处理
     * @param $string
     * @return string
     */
    ...
    public function des_encode($str){
        ...
        return $this->strToHex(mcrypt_encrypt(MCRYPT_DES, $this->getConf('des_key',__CLASS__), $str, MCRYPT_MODE_ECB));
    }
    ...
    public function callback(&$recv) {
        \Neigou\Logger::Debug('callback.mminshengyixiang.req', array('remark' => 'recv_param', 'post_data'=>$recv));
        ...
        if($this->is_return_vaild($recv)){
            ...
            $ret['pay_type'] = 'online';
            $ret['status'] = 'succ';
            \Neigou\Logger::General('ecstore.callback.mminshengyixiang', array('remark' => 'trade_succ', 'data' => $ret));
        }else{
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.callback.mminshengyixiang',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    }
    ...
    public function genSign($postData) {
        ...
        \Neigou\Logger::General('paysign.mminshengyixiang',array('linkS'=>$sign_str));
...
eot;
    ...
    public function is_return_vaild($param) {
        ...
        if($sign==md5($str)){
            ...
            \Neigou\Logger::General('callback.mminshengyixiang.sign.err',array('linkS'=>$str,'sign'=>md5($str),'sign_req'=>$sign));
            return false;
        }
    }
