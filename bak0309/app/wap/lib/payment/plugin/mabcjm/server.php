...
class wap_payment_plugin_mabcjm_server extends ectools_payment_app
{

    /**
     * 支付后返回后处理的事件的动作
     * https://pay.test.abchina.com/easyebus/#/api/pay/payment-result-notify/abc-payment-result-notify
    ...
    public function callback( &$recv )
    {
        ...
        \Neigou\Logger::General( 'mabcjm', array('action'=>'init', 'remark' => '异步通知开始', 'post_data' => $recv ) );
        ...
        {
            $mer_id = $this->getConf( 'mer_id', 'wap_payment_plugin_mabcjm' );

            \Neigou\Logger::General( 'mabcjm', array( 'action' => 'base64', 'remark' => '交易细节', 'payment_id' => $ret, 'recv' => $recv, 'tmessage' => $tMessage ) );
            ...
            $ret['status'] = 'succ';
            echo 'succ';
            \Neigou\Logger::General( 'mabcjm', array( 'remark' => 'trade_succ', 'data' => $ret ) );
        }
        else
        {
            echo 'sign_err';
            $ret['status'] = 'invalid';
            \Neigou\Logger::General( 'mabcjm', array( 'remark' => 'sign_err', 'data' => $recv ) );
        }
        return $ret;
    ...
    {
        $tMessage = "{\"Version\":\"V3.0.0\",\"Format\":\"JSON\",\"Merchant\":" . "{\"ECMerchantType\":\"" . "EBUS" . "\",\"MerchantID\":\"" . $this->getConf( 'mer_id', 'wap_payment_plugin_mabcjm' ) . "\"}," . "\"TrxRequest\":" . $aMessage . "}";
        return $tMessage;
    }
    ...
    {
        //1、读取商户证书
        $tMerchantCertFile = $this->getConf( 'pfx_file', 'wap_payment_plugin_mabcjm' );
        $tMerchantCertPassword = $this->getConf( 'pfx_file_pass', 'wap_payment_plugin_mabcjm' );
        //2、读取证书
        if ( openssl_pkcs12_read( file_get_contents( $tMerchantCertFile ), $tCertificate, $tMerchantCertPassword ) )
        {
            ...
            {
                \Neigou\Logger::General( 'mabcjm', array( 'remark' => 'pfx证书过期' ) );
            }
            //4、取得密钥
            $pkey = openssl_pkey_get_private( $tCertificate['pkey'] );
            if ( !$pkey )
            {
                \Neigou\Logger::General( 'mabcjm', array( 'remark' => '无法生成私钥证书对象' ) );
            }
        }
        else
        {
            \Neigou\Logger::General( 'mabcjm', array( 'remark' => '证书读取失败' ) );
        }
        $signature = '';
    ...
    public function genSignature( $tRequestMessage )
    {
        ...
        \Neigou\Logger::General( 'mabcjm', array('action'=>'request sign', 'remark' => '交易报文', 'req_str' => $tRequestMessage, 'sign_str' => $sign_str ) );
        return $sign_str;
    }
