 ...
 * Time: 15:25
 */
final class wap_payment_plugin_mabcchinafq extends ectools_payment_app implements ectools_interface_payment_app
{
    /**
     * @var string 支付方式名称
     */
     ...
     * @var string 支付方式key
     */
    public $app_key = 'mabcchinafq';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'mabcchinafq';
    /**
     * @var string 统一显示的名称
     */
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mabcchinafq_server', 'callback');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mabcchinafq', 'callback');
    ...
    public function setting(){
        return array(
            ...
            'InstallmentCode'=>array(
                'title'=>app::get('ectools')->_('分期代码'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'InstallmentNum'=>array(
                'title'=>app::get('ectools')->_('分期期数'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'submit_url'=>array(
    ...
    public function dopay($payment) {
        //设定订单属性
        $order['PayTypeID'] = 'DividedPay';//交易类型 直接支付
        ...
        $order['InstallmentMark'] = '1';//分期标志 1分期 0 不分期
        $order['InstallmentCode'] = $this->getConf('InstallmentCode',__CLASS__);//分期代码
        $order['InstallmentNum'] = $this->getConf('InstallmentNum',__CLASS__);//分期期数
...
//        print_r($pay_obj);die;
        ...
        $request['PaymentType'] = '3';//支付类型  1：农行卡支付 2：国际卡支付 3：农行贷记卡支付 5：基于第三方的跨行支付 6：银联跨行支付 7：对公户 A:支付方式合并 TODO 用户是否可以自己修改
        ...
        if($this->is_return_vaild($res)){
            ...
            $req_url = 'https://www.abchina.com/luascript/mobilePayLua/{"return":{"tokenID":"'.$token.'","backURL":"'.$this->callback_url.'"}}';
            \Neigou\Logger::General('req.mabcchinafq.url',array('remark'=>'支付TOKEN','token'=>$token,'order_id'=>$payment['order_id']));
    ...
    public function signMessage($aMessage) {
        ...
        if (openssl_pkcs12_read(file_get_contents($tMerchantCertFile), $tCertificate, $tMerchantCertPassword)) {
            ...
            if ($t < $cer['validFrom_time_t'] || $t > $cer['validTo_time_t']) {
                \Neigou\Logger::General('sign.mabcchinafq.pfx_file', array('remark' => 'pfx证书过期'));
            }
            //4、取得密钥
            $pkey = openssl_pkey_get_private($tCertificate['pkey']);
            if (!$pkey) {
                \Neigou\Logger::General('sign.mabcchinafq.pfx_file', array('remark' => '无法生成私钥证书对象'));
            }
        } else {
            \Neigou\Logger::General('sign.mabcchinafq.pfx_file', array('remark' => '证书读取失败'));
        }
        $signature = '';
...
//        $tMessage = str_replace('"', "&quot;", $tMessage);//TODO 网站提交需
    ...
    public function genSignature($tRequestMessage) {
        ...
        \Neigou\Logger::General('pay.mabcchinafq',array('remark'=>'交易报文','req_str'=>$tRequestMessage,'sign_str'=>$sign_str));
        return $sign_str;
    }
    ...
    public function callback(&$recv) {
        ...
        \Neigou\Logger::General('ecstore.callback.mabcchinafq',array('remark'=>'请求参数','field'=>json_encode($field),'tSignature'=>$tSignature,'response_data'=>$res));
        ...
        if($this->is_return_vaild($res) && $pay_status=='0000'){
            ...
            $ret['t_payed'] = time();
            $ret['pay_app_id'] = "mabcchinafq";
            $ret['pay_type'] = 'online';
            $ret['status'] = 'succ';
            \Neigou\Logger::General('ecstore.callback.mabcchinafq', array('remark' => 'trade_succ', 'data' => $ret));
        }else{
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.callback.mabcchinafq',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    }
...
<form name="form2" method="post" action="{$this->submit_url}">
...
eot;
    ...
    public function is_return_vaild($aMessage) {
        ...
        $iTrustpayCertificate = openssl_x509_read($this->der2pem(file_get_contents($this->getConf('trustPay_file','wap_payment_plugin_mabcchinafq'))));
