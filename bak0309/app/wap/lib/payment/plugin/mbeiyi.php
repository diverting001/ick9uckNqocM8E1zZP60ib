...
final class wap_payment_plugin_mbeiyi extends ectools_payment_app implements ectools_interface_payment_app
{
    /**
     * @var string 支付方式名称
     */
    public $name = 'wap 北移娘家人支付';
    /**
     * @var string 支付方式接口名称
     */
    public $app_name = 'wap 北移娘家人支付';
    /**
     * @var string 支付方式key
     */
    public $app_key = 'mbeiyi'; // 重要
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'mbeiyi'; // 重要
    /**
     * @var string 统一显示的名称
     */
    public $display_name = 'wap 北移娘家人支付';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    public function __construct($app)
    {
        ...
        // 异步通知
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mbeiyi_server', 'callback');
        ...
        // 同步通知
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mbeiyi', 'callback');
    ...
    public function dopay($payment)
    {
        try {
            ...
            Neigou\Logger::General('ecstore.mbeiyi_pay',array('sub_name' => 'request','fields' => $this->fields,'method' => __METHOD__));
            // 请求url地址
            $url = $this->_config['submit_url'].'?Attach=byddshop&byddshop='.$encryStr;
        } catch (Exception $e) {
            \Neigou\Logger::General('ecstore.mbeiyi_pay_error', array('errorMsg' => $e->getMessage(), 'order_id' => $payment['order_id'], 'env' => 'wap端', 'payment' => $payment));
    ...
    private function setRequestStr($data = array()){
        $str = $data['OrderSN'].':'.urlencode($data['body']).':'.$data['TotalFee'].':'.$data['UserOpenId'].':'.$data['timestamp'].':'.$data['payment_id'];
        return $str;
    }
