...
<a target="_blank" href="https://www.alipay.com/static/utoj/utojindex.htm">'.app::get('wap')->_('如何使用支付宝支付？').'</a>';
    ...
    public function dopay($payment){
        ...
        $params['partner'] = $mer_id;
        $params['_input_charset'] = $this->_input_charset;
        $params['notify_url'] = $this->notify_url;
        $params['return_url'] = $this->callback_url;
        $params['out_trade_no'] = $payment['payment_id'];
        $params['subject'] = $subject_tmp;
        $params['total_fee'] = $price;
        $params['seller_id'] = $mer_id;
        $params['payment_type'] = 1;
        $params['show_url'] = kernel::base_url(1) . '/m/member-new_orders-nopayed.html';
        //判断公司是否是24小时可以支付的
        ...
        if($expire>0){
            $params['it_b_pay'] = floor($expire/60).'m';
        }
        ...
        if(in_array($member_info['company_id'],explode(',',NO_MALIPAY_APP)) || 
            ...
            unset($params['app_pay']);
        }


        $get = $this->para_filter($params);
        $sort_get = $this->arg_sort($get);
        $my_sign = $this->build_mysign($sort_get,$mer_key,strtoupper(trim($this -> sec_id)));
        $params['sign'] = $my_sign;
        $params['sign_type'] = strtoupper(trim($this -> sec_id));
