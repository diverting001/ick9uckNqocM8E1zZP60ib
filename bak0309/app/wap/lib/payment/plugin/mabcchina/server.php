 ...
 * Time: 10:31
 */
class wap_payment_plugin_mabcchina_server extends ectools_payment_app {
    ...
    public function callback(&$recv) {
        ...
        \Neigou\Logger::General('notify.mabcchina.req', array('remark' => '异步通知开始', 'post_data'=>$recv));
        $aMessage = $recv['MSG'];
        //1、还原经过base64编码的信息
        \Neigou\Logger::General('notify.mabcchina.req', array('remark' => '异步通知aMessage', 'post_data'=>$aMessage));
        $tMessage = base64_decode(trim($aMessage));
        //2、取得经过签名验证的报文
        \Neigou\Logger::General('notify.mabcchina.req', array('remark' => '解析base64', 'post_data'=>iconv('GB2312','UTF-8',$tMessage)));
        $mer_id = $this->getConf('mer_id', 'wap_payment_plugin_mabcchina');
        if($this->is_return_vaild($tMessage)){
            \Neigou\Logger::General('notify.mabcchina.req', array('remark' => '交易签名认证通过', 'post_data'=>1));
            $ret['payment_id'] = $this->getValueXml('OrderNo',$tMessage);
            \Neigou\Logger::General('notify.mabcchina.tmp', array('remark' => '交易细节', 'payment_id'=>$ret['payment_id']));
            $ret['account'] = $mer_id;
            ...
            $ret['t_payed'] = time();
            $ret['pay_app_id'] = "mabcchina";
            $ret['pay_type'] = 'online';
            $ret['status'] = 'succ';
            echo 'succ';
            \Neigou\Logger::General('ecstore.notify.mabcchina', array('remark' => 'trade_succ', 'data' => $ret));
        }else{
            echo 'sign_err';
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.notify.mabcchina',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    ...
    public function query(){
        ...
        \Neigou\Logger::General('ecstore.notify.mabcchina',array('remark'=>'请求参数','field'=>json_encode($field),'tSignature'=>$tSignature,'response_data'=>$res));
    ...
    private function composeRequestMessage($aMessage) {
        $tMessage = "{\"Version\":\"V3.0.0\",\"Format\":\"JSON\",\"Merchant\":" . "{\"ECMerchantType\":\"" . "EBUS" . "\",\"MerchantID\":\"" . $this->getConf('mer_id','wap_payment_plugin_mabcchina') . "\"}," . "\"TrxRequest\":" . $aMessage . "}";
        return $tMessage;
    }

    public function signMessage($aMessage) {
        //1、读取商户证书
        $tMerchantCertFile = $this->getConf('pfx_file','wap_payment_plugin_mabcchina');
//        echo $tMerchantCertFile;die;
        $tMerchantCertPassword = $this->getConf('pfx_file_pass','wap_payment_plugin_mabcchina');
        //2、读取证书
        if (openssl_pkcs12_read(file_get_contents($tMerchantCertFile), $tCertificate, $tMerchantCertPassword)) {
            ...
            if ($t < $cer['validFrom_time_t'] || $t > $cer['validTo_time_t']) {
                \Neigou\Logger::General('sign.mabcchina.pfx_file', array('remark' => 'pfx证书过期'));
            }
            //4、取得密钥
            $pkey = openssl_pkey_get_private($tCertificate['pkey']);
            if (!$pkey) {
                \Neigou\Logger::General('sign.mabcchina.pfx_file', array('remark' => '无法生成私钥证书对象'));
            }
        } else {
            \Neigou\Logger::General('sign.mabcchina.pfx_file', array('remark' => '证书读取失败'));
        }
        $signature = '';
...
//        echo $tRequestMessage;die;
        ...
        \Neigou\Logger::General('pay.mabcchina',array('remark'=>'交易报文','req_str'=>$tRequestMessage,'sign_str'=>$sign_str));
        return $sign_str;
    }
