 ...
 */
class wap_payment_plugin_mcaibei_server extends ectools_payment_app {
    ...
    public function callback(&$recv) {
        ...
        \Neigou\Logger::General('ecstore.notify.mcaibei',array('remark'=>'notify_param','data'=>$recv));
        ...
        if($this->is_return_vaild($recv)){
            ...
            echo 'fail';
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.notify.mcaibei',array('remark'=>'sign err','data'=>$recv));
        }
        return $ret;
    }
    ...
    public function is_return_vaild($param) {
        $sign = $param['sign'];
        unset($param['sign']);
        $param['salt'] = $this->getConf('salt','wap_payment_plugin_mcaibei');
    ...
    public function genSign($data){
        ...
        \Neigou\Logger::General('ecstore.mcaibei.notify.linkStr',array('action'=>'make sign','linkStr'=>$linkStr,'sign'=>md5($linkStr)));
        return md5($linkStr);
    }
