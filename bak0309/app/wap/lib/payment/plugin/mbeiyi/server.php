...
class wap_payment_plugin_mbeiyi_server extends ectools_payment_app
{
    ...
    public function callback(&$recv)
    {
        ...
        \Neigou\Logger::Debug('notify.beiyi.req', array('remark' => 'recv_param', 'post_data' => $recv,'method' => __METHOD__));
        ...
        try{
            ...
            $sign =  kernel::single('wap_payment_plugin_mbeiyi')->encrypt($signBeforeStr);
            if ($sign != $recv['sign']){
                throw new Exception('签名验证失败');
            }

            $ret['bank'] = app::get('ectools')->_('北移娘家人支付');
            $ret['currency'] = 'CNY';
            ...
            $ret['pay_app_id'] = "mbeiyi";
            $ret['pay_type'] = 'online';
            $ret['status'] = 'succ';
            $ret['callback_source'] = 'server';
            \Neigou\Logger::General('ecstore.notify.beiyi', array('remark' => 'trade_succ', 'data' => $ret,'recv' => $recv));
            echo 'succ';
        }catch (Exception $e){
            \Neigou\Logger::General('ecstore.notify.beiyi',array('report_name' => 'ecstore.notify.beiyi.error','error_msg' => $e->getMessage(),'data' => $recv,'recv' => $recv));
