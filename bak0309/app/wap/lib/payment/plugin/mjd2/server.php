 ...
 */
class wap_payment_plugin_mjd2_server extends ectools_payment_app {
    ...
    public function callback(&$recv) {
        \Neigou\Logger::General('ecstore.notify.mjd2', array('remark' => 'param_init', 'data' => 111,'post_data'=>$GLOBALS["HTTP_RAW_POST_DATA"],'request_data'=>$_REQUEST));
        ...
        if($this->is_return_vaild($GLOBALS["HTTP_RAW_POST_DATA"],$recv)){
            if($recv['status']==2){
                ...
                $ret['status'] = 'succ';
                echo 'success';
                \Neigou\Logger::General('ecstore.notify.mjd2', array('remark' => 'trade_succ', 'data' => $recv));
            } else {
                $ret['status']='invalid';
                echo 'fail';
                \Neigou\Logger::General('ecstore.notify.mjd2', array('remark' => 'trade_status_err', 'data' => $recv));
            }
        }else{
            echo 'fail';
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.notify.mjd2',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    ...
    public function is_return_vaild($resultData,&$resData) {
        ...
        \Neigou\Logger::General('ecstore.notify.mjd2',array('remark'=>'reqBody','data'=>$reqBody));
        $bodyXml = simplexml_load_string($reqBody);
        //echo "请求返回encrypt Des解密后:".$bodyXml->saveXML()."\n";
        \Neigou\Logger::General('ecstore.notify.mjd2',array('remark'=>'bodyXml','data'=>$bodyXml));
...
// 		$reqBodyStr = XMLUtil::xmlToString($bodyDom);
        ...
        //echo "本地摘要原串:".$xml."\n";
        \Neigou\Logger::General('ecstore.notify.mjd2',array('remark'=>'本地摘要原串','data'=>$xml));
        $sha256SourceSignString = hash("sha256", $xml);
        \Neigou\Logger::General('ecstore.notify.mjd2',array('remark'=>'本地摘要','data'=>$sha256SourceSignString));
        //echo "本地摘要:".$sha256SourceSignString."\n";

        $decryptStr = $this->decryptByPublicKey($inputSign);
        \Neigou\Logger::General('ecstore.notify.mjd2',array('remark'=>'解密后摘要','data'=>$decryptStr));
        //echo "解密后摘要:".$decryptStr."\n";
        $flag;
        if($decryptStr==$sha256SourceSignString){
            //echo "验签成功<br/>";
            \Neigou\Logger::General('ecstore.notify.mjd2',array('remark'=>'sign_true'));
            $flag=true;
        }else{
            //echo "验签失败<br/>";
            \Neigou\Logger::General('ecstore.notify.mjd2',array('remark'=>'sign_err'));
            $flag=false;
        }
        $resData["version"]=$resultObj["version"];
        $resData["merchant"]=$resultObj["merchant"];
        $resData["result"]=$resultObj["result"];
        \Neigou\Logger::General('ecstore.notify.mjd2',array('remark'=>'resData','data'=>$resData));
