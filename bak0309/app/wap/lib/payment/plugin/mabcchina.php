 ...
 * Time: 15:25
 */
final class wap_payment_plugin_mabcchina extends ectools_payment_app implements ectools_interface_payment_app
{
    /**
     * @var string 支付方式名称
     */
     ...
     * @var string 支付方式key
     */
    public $app_key = 'mabcchina';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'mabcchina';
    /**
     * @var string 统一显示的名称
     */
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mabcchina_server', 'callback');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mabcchina', 'callback');
...
//        print_r($pay_obj);die;
        ...
        if($this->is_return_vaild($res)){
            ...
            \Neigou\Logger::General('req.mabcchina.url',array('remark'=>'支付TOKEN','token'=>$token,'order_id'=>$payment['order_id']));
...
//        $tMessage = str_replace('"', "&quot;", $tMessage);//TODO 网站提交需
    ...
    public function callback(&$recv) {
        ...
        \Neigou\Logger::General('ecstore.callback.mabcchina',array('remark'=>'请求参数','field'=>json_encode($field),'tSignature'=>$tSignature,'response_data'=>$res));
        ...
        if($this->is_return_vaild($res) && $pay_status=='0000'){
            ...
            $ret['pay_type'] = 'online';
            $ret['status'] = 'succ';
            \Neigou\Logger::General('ecstore.callback.mabcchina', array('remark' => 'trade_succ', 'data' => $ret));
        }else{
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.callback.mabcchina',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    }
...
<CENTER>支付请求<br><form name="form2" method="post" action="{$this->submit_url}">
