 ...
 * Time: 16:18
 */
final class wap_payment_plugin_mjuyoucps extends ectools_payment_app implements ectools_interface_payment_app
{
    /**
     * @var string 支付方式名称
     */
    public $name = '聚优cps 支付';
    /**
     * @var string 支付方式接口名称
     */
    public $app_name = '聚优cps 支付';
    /**
     * @var string 支付方式key
     */
    public $app_key = 'mjuyoucps';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'mjuyoucps';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '聚优cps 支付';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
     ...
     * @var array
     */
    public $auto_submit_channel = array('jyoto');
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mjuyoucps_server', 'callback');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mjuyoucps', 'callback');
    ...
    public function setting(){
        return array(
            ...
            'mer_id'=>array(
                'title'=>app::get('ectools')->_('商户号 聚优OTO提供的商户号'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'client_id'=>array(
            ...
            'salt'=>array(
                'title'=>app::get('ectools')->_('聚优OTO服务salt'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'submit_url'=>array(
    ...
    public function intro(){
        return app::get('ectools')->_('聚优CPS 支付配置信息');
    }
    ...
    public function dopay($payment) {
        ...
        $cps_bill_id = '';
        //1.查询是否是跳转回来的请求
        $_redis->fetch('payment_juyoucps_',$payment['order_id'],$cps_bill_id);
        if(empty($cps_bill_id)){
            $_redis->store('payment_juyoucps_',$payment['order_id'],$payment['payment_id'],3000);
        } else {
            $payment['payment_id'] = $cps_bill_id;
        }
    ...
    public function callback(&$recv) {
        if($this->is_return_vaild($recv)){
            ...
            $ret['bank'] = app::get('ectools')->_('聚优CPS支付');
            $ret['pay_account'] = '聚优cps';
            $ret['currency'] = 'CNY';
            ...
            $ret['t_payed'] = time();
            $ret['pay_app_id'] = "mjuyoucps";
            $ret['pay_type'] = 'online';
            $ret['status'] = 'succ';
        }else{
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.callback.mjuyoucps',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    }
    ...
    public function genSign($data){
        ...
        \Neigou\Logger::General('ecstore.mjuyoucps.linkStr',array('action'=>'make sign','linkStr'=>$linkStr,'sign'=>md5($linkStr)));
        return md5($linkStr);
    }
    ...
    public function request($api = '', $post_data = array())
    {
        ...
        echo $result;
        \Neigou\Logger::General("ecstore.sync.mjuyoucps",
