 ...
 */
class wap_payment_plugin_mwxqwomai_server extends ectools_payment_app {
    ...
    public function callback(&$recv) {
        ...
        \Neigou\Logger::General('ecstore.notify.mwxqwomai',array('remark'=>'notify_param','data'=>$recv,'raw_data'=>$GLOBALS["HTTP_RAW_POST_DATA"],'request_data'=>$_REQUEST));
        #键名与pay_setting中设置的一致
        if($this->is_return_vaild($recv)){
            $payment_id = str_replace('QYDD','',$recv['out_trade_no']);
            ...
            if($recv['trade_state']=='0') {//为0 代表支付结果成功
                ...
                $this->msg(false);
                $ret['status'] = 'failed';
                \Neigou\Logger::General('ecstore.notify.mwxqwomai',array('remark'=>'pay rzt err','data'=>$recv));
            }
        }else{
            $this->msg(false);
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.notify.mwxqwomai',array('remark'=>'sign err','data'=>$recv));
        }
        return $ret;
    }
    ...
    public function redirect(){
        ...
        $des = $this->decrypt($req,$this->getConf('code','wap_payment_plugin_mwxqwomai'));
        ...
        if(!empty($openId)){
            ...
            $_redis->store('payment_mwxqwomai_open_id_',trim($_REQUEST['payment_id']),$openId,300);
        } else {
            //设置微信支付参数为1
            $_redis->store('payment_mwxqwomai_open_id_',trim($_REQUEST['payment_id']),1,300);
        }
        //跳转回原支付

        $payment_id = trim($_REQUEST['payment_id']);
        $payment_id = str_replace('QYDD','',$payment_id);
        ...
        $fields = array();
        $_redis -> fetch('payment_mwxqwomai_info_',$payment_id,$fields);
        if(!empty($fields)){
        ...
        $_redis->store('payment_mwxqwomai_payment_id_',$sdf['rel_id'],$payment_id,300);
    ...
    public function query(){
        ...
        if($this->is_return_vaild($_POST)){
            if ($sdf){
                $out['errcode'] = 'SUCCESS';
                $out['errmsg'] = '成功';
                $out['body'] = 'QYDD-'.$sdf['payment_id'];
                $out['attach'] = $sdf['payment_id'];
                $out['out_trade_no'] = 'QYDD'.$sdf['payment_id'];
        ...
        \Neigou\Logger::General('ecstore.notify.mwxqwomai.queryOrder',array('remark'=>'query_order','out'=>$out,'raw_data'=>$GLOBALS["HTTP_RAW_POST_DATA"],'request_data'=>$_REQUEST));
    ...
    public function genSign($data){
        ...
        $linkStr .= 'key='.$this->getConf('md5_key','wap_payment_plugin_mwxqwomai');
        \Neigou\Logger::General('ecstore.mwxqwomai.notify.linkStr',array('action'=>'make sign','linkStr'=>$linkStr,'sign'=>md5($linkStr)));
