 ...
 * Time: 15:25
 */
final class wap_payment_plugin_mabcchinahtmlmx extends ectools_payment_app implements ectools_interface_payment_app
{
    /**
     * @var string 支付方式名称
     */
    public $name = '农行H5免息支付';
    /**
     * @var string 支付方式接口名称
     */
    public $app_name = '农行H5免息支付';
    /**
     * @var string 支付方式key
     */
    public $app_key = 'mabcchinahtmlmx';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'mabcchinahtmlmx';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '农行H5免息支付';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mabcchinahtmlmx_server', 'callback');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mabcchinahtmlmx', 'callback');
    ...
    public function setting(){
        return array(
            ...
            'InstallmentCode'=>array(
                'title'=>app::get('ectools')->_('分期项目代码'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
    ...
    public function intro(){
        return app::get('ectools')->_('农行H5免息支付');
    }
...
//        $tMessage = str_replace('"', "&quot;", $tMessage);//TODO 网站提交需
    ...
    public function callback(&$recv) {
        ...
        \Neigou\Logger::General('ecstore.callback.mabcchinahtmlmx',array('remark'=>'请求参数','field'=>json_encode($field),'tSignature'=>$tSignature,'response_data'=>$res));
        ...
        if($this->is_return_vaild($res) && $pay_status=='0000'){
            ...
            $ret['pay_type'] = 'online';
            $ret['status'] = 'succ';
            \Neigou\Logger::General('ecstore.callback.mabcchinahtmlmx', array('remark' => 'trade_succ', 'data' => $ret));
        }else{
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.callback.mabcchinahtmlmx',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    }
