 ...
 * Time: 15:39
 */
final class wap_payment_plugin_mjd extends ectools_payment_app implements ectools_interface_payment_app
{
    /**
     * @var string 支付方式名称
     */
     ...
     */
    public $app_rpc_key = 'mjd';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '京东标准支付';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mjd_server', 'callback');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_mjd', 'callback');
    ...
    public function intro(){
        return app::get('ectools')->_('京东标准支付');
    }
    ...
    public function dopay($payment) {
        ...
        \Neigou\Logger::General('pay.jd', array('action' => 'dopayjd', 'sign_fields' => $sign_fields,'des_fields'=>$this->fields));
        echo $this->get_html();
        exit();
    }
    ...
    public function callback(&$recv) {
        ...
        if($valid['status']=='true'){
            ...
            if($recv['status']==0){
                ...
                $ret['pay_type'] = 'online';
                $ret['status'] = 'succ';
                \Neigou\Logger::General('pay.mjd', array('action' => 'callback_succ', 'data' => $recv));
            } else {
                \Neigou\Logger::General('pay.mjd', array('action' => 'trade_status_err', 'data' => $recv));
                $ret['status'] = 'invalid';
            }

        }else{
            \Neigou\Logger::General('pay.mjd', array('action' => 'sign_err', 'data' => $recv));
            $ret['status'] = 'invalid';
        }
        return $ret;
    }
...
eot;
    ...
    public function is_return_vaild($params) {
        ...
        \Neigou\Logger::General('pay.jd', array('action' => 'jdcallback', 'sign_fields' => $strSourceData,'sign_jd'=>$decryptStr,'sign_mine'=>$sha256SourceSignString,'param'=>$params,'des_param'=>$param));
