 ...
 */
class wap_payment_plugin_mmcdonld_server extends ectools_payment_app {
	/**
	 * 支付后返回后处理的事件的动作
    ...
    public function callback(&$recv){
        \Neigou\Logger::General('ecstore.notify.mmcdonld', array('remark' => 'param_init','request_data'=>$_REQUEST));
        ...
        if($this->is_return_vaild($recv,'RSA2')){
            ...
            if($recv['trade_status'] == 'TRADE_FINISHED' || $recv['trade_status'] == 'TRADE_SUCCESS'){
                ...
                $ret['account'] = $mer_id;
                $ret['bank'] = app::get('ectools')->_('麦当劳支付');
                ...
                $ret['pay_app_id'] = "mmcdonld";
                $ret['pay_type'] = 'online';
                $ret['status'] = 'succ';
                echo 'success';
                \Neigou\Logger::General('ecstore.notify.mmcdonld', array('remark' => 'trade_succ', 'data' => $recv));
            } else {
                $ret['status']='invalid';
                echo 'fail';
                \Neigou\Logger::General('ecstore.notify.mmcdonld', array('remark' => 'trade_status_err', 'data' => $recv));
            }
        }else{
            echo 'fail';
            $ret['status'] = 'invalid';
            \Neigou\Logger::General('ecstore.notify.mmcdonld',array('remark'=>'sign_err','data'=>$recv));
        }
        return $ret;
    }
    ...
    public function is_return_vaild($params,$signType='RSA') {
        ...
        if(!$ret){
            \Neigou\Logger::General('ecstore.notify.mmcdonld', array('action' => 'trade_start', 'data' => json_encode($params),'data2'=>$signType,'sparam1'=>json_encode($this->zfb_public_key)));
        }
        return $ret;
    }
