...
var MooCrop = new Class({
	...
	refresh : function(event){
		var xdiff = this.current.x - event.pageX;
		var ydiff = this.current.y - event.pageY;
	...
	buildOverlay: function(){
		...
		if(this.options.showMask){		// optional masks
			...
			this.north = new Element("div", {'styles' : maskStyles}).inject(this.wrapper,'bottom');
			this.south = new Element("div", {'styles' : $merge(maskStyles,{'bottom':'0px'})}).inject(this.wrapper,'bottom');
			this.east =  new Element("div", {'styles' : maskStyles}).inject(this.wrapper,'bottom');
			this.west =  new Element("div", {'styles' : maskStyles}).inject(this.wrapper,'bottom');
		}
