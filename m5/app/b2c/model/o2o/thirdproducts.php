...
class b2c_mdl_o2o_thirdproducts extends base_db_external_model
{
    ...
    private function send_third_coupon($bn, $num, $sdf)
    {
        ...
        $hidePhone  =  kernel::single("b2c_global_scope")->getScopeByWeight('checkout_o2o_hide_phone', '', $sdf['company_id']);

        $serviceOrderInfo = kernel::single('b2c_service_order')->getOrderInfo($sdf['order_id']);
        $wmsCode = $serviceOrderInfo['wms_code'];
        // 联动票务
        if ($supplierBn === 'LDPW')
        {
        ...
        // SHOP 推消费码
        elseif (in_array($wmsCode, array('SHOP', 'SHOPNG')))
        {
...
//            $result = $this->supplierSendCoupon($bn, $this->_supportSuppliers[$supplierBn], $num, $members['member_id'], $sdf, $orderExtend);
            $result = array(
                'result' => true,
                'data'   => array(
                    'supplier_order_id' => $sdf['order_id'],
                    'is_async'          => 1, //异步处理
                ),
            );
            // 供应商订单 ID
    ...
    public function retrySend($order_id, $member_id, $bn, $name, $mobile,$message_channel = "")
    {
        ...
        // 获取供应商的BN
        $supplierBn = self::_getSupplierBn($bn);
        $supplierName = $this->_supportSuppliers[$supplierBn];

        $serviceOrderInfo = kernel::single('b2c_service_order')->getOrderInfo($order_id);
        $wmsCode = $serviceOrderInfo['wms_code'];
        ...
        }elseif (in_array($supplierBn, array('TS', 'RY', 'ZYKM', 'FLKM','BGY','FLKMV2')))
        {
            $info = app::get("b2c")->model('o2o_products')->getLdInfoByOrderId($order_id, $member_id, $bn);
            if (empty($info))
            {
            ...
            $templateConfig = kernel::single('b2c_service_messagecenter')->getCompanyMessageTemplate($serviceOrderInfo['company_id'], 'salyut_o2o_coupon');
            if (empty($templateConfig)) {
                return json_encode(array("Result" => 'false', "ErrorMsg" => '短信模板不存在', 'ErrorId' => '10000'));
            }

            foreach($info as $key=>$val){
                if (!empty($val['aux_code'])) {
                    $sendData = array(
                        'channel' => $templateConfig['channel_name'],
                        'data' => array(
                            array(
                                'receiver' => array($mobile),
                                'template_id' => $templateConfig['template_id'],
                                'template_param' => array('name' => $name, 'numbers' => $val['aux_code']),
                            ),
                        ),
                    );

                    //配置替换
                    $replaceChannel = $this->_O2OSmsChannelReplaceByParam(implode(',', $sendData['data'][0]['template_param']), $sendData['channel'], 'salyut');
                    if (!empty($replaceChannel)) {
                        $sendData['channel'] = $replaceChannel;
                    }
        ...
        if (!empty($message_channel) && $type == 'reSend' && $result['result'] == 'true' && !empty($result['data']) && is_array($result['data'])) {
            $sendData = array(
                ...
                'data' => array(
                    array(
                        'receiver' => array($num),
                        'template_id' => O2O_VIRTUAL_SMS_TEMPLATE_ID,
            ...
            foreach ($result['data'] as $datum) {
                if ($datum['ticket_code']) {
                    ...
                    //配置替换
                    $replaceChannel = $this->_O2OSmsChannelReplaceByParam(implode(',', $sendData['data'][0]['template_param']), $sendData['channel'], 'shop');
                    if (!empty($replaceChannel)) {
                        $sendData['channel'] = $replaceChannel;
                    }
                    $this->sendMessage($sendData);
     ...
     * @return array|mixed
     */
    private function _O2OSmsChannelReplaceByParam($templateParam, $channel, $supplier) {
        $replaceChannel = '';

        //链接检测开启
        if ($channel != '内'.'购短信' || !O2O_SMS_LINK_DETECTION) {
            return $replaceChannel;
        }

        //检测参数是否有链接
        if (!$this->_hasLink($templateParam)) {
            return $replaceChannel;
        }

        //获取默认短信配置
        if ($supplier == 'shop') {
            $config = $this->_getShopDefaultCouponSMSConfig();
        } else if($supplier == 'salyut') {
            $config = $this->_getSalyutDefaultCouponSMSConfig();
        }
     ...
     * @return mixed
     */
    private function _getShopDefaultCouponSMSConfig() {
        $config = kernel::single('b2c_config')->getValueByKey('shop_o2o_coupon_sms_set');
     ...
     * @return mixed
     */
    private function _getSalyutDefaultCouponSMSConfig() {
        $config = kernel::single('b2c_config')->getValueByKey('salyut_o2o_coupon_sms_set');
        $config = json_decode($config, true);
        return $config ?: array();
    }
     ...
     * @return bool
     */
    private function _hasLink($str) {
        $pattern = '/(?:http|https):\/\/[^\s]*[.]([a-z]{2,})[^\s]*/i';
    ...
    /**
     * Notes:获取券码信息
     * Date: 2024/12/11 上午10:15
     * @param $bn
     * @param $couponNo
     * @return array|false|mixed
     */
    public function getCouponInfoByCouponNo($bn, $couponNo)
    {
        if (empty($bn) || empty($couponNo)) {
            return false;
        }
        // 获取第三方编码
        $third_product = app::get("b2c")->model('o2o_products')->get_third_product_bn($bn);
        ...
        $requestParam = array(
            'class_obj' => 'O2O',
            'method' => 'order',
            'action' => 'getCouponInfoByCouponNo', // 操作
            'supplier' => $this->_supportSuppliers[$supplierBn], // 供应商
            'data' => array(
                // 订单信息
                'order' => array(
        ...
        \Neigou\Logger::General("o2o_thirdproducts.order.prepare", array('action' => 'getCouponInfoByCouponNo', 'requestData' => json_encode($requestParam), 'result' => $result));
    ...
    /**
     * Notes:校验充值账号
     * Date: 2024/12/11 上午11:46
     * @param $bn
     * @param $couponNo
     * @return array|false|mixed
     */
    public function checkActivateCoupon($orderId,$couponNo, $bn, $mobile, $account)
    {
        if (empty($orderId) || empty($couponNo) || empty($bn)) {
            return false;
        }
        ...
        \Neigou\Logger::General("o2o_thirdproducts.order.prepare", array('action' => 'checkActivateCoupon', 'requestData' => json_encode($requestParam), 'result' => $result));
